...
---
<!-- Cloned from claude/skills/all-hands/SKILL.md. Do not edit here; change the source and re-copy; enforce/tests/skills-lint.test.sh fails when this copy drifts. -->
...
> You are running a lightweight weekly priorities scan, not a full audit. Read the codebase quickly. Return ONLY your top 3-5 action items for this week, each as a single bullet: `[P0/P1/P2] <role>: <action item in one sentence>`. The R-804 evidence requirement (pasted code with file:line) is waived for this format; each bullet still names the file, directory, or surface it is about, and resolves precedence before flagging (a documented override is not a finding). Do not write a full audit report. Do not write to any file. Return the bullets only.
...
**Step 2 -- fan out.** Dispatch the remaining 8 roles in a single parallel batch, each with the same lightweight prompt above (substituting their role name). Pass `model: sonnet` on every dispatch: the role file's own `model` line (Opus for engineering, security, criticism, and customer) is the default for a full audit, not for this scan, and the Agent tool applies the role file's model unless the call overrides it.
...
Commit the file to the current branch. Do not create a separate branch. Route the items per R-802: a P0 goes to the current effort now; a P1 or P2 waits for that role's full audit, or goes to `ISSUES.md` with its priority tag when no audit is scheduled.
...
Sonnet for every role, passed as `model: sonnet` on each dispatch (the role files set Opus for the four standing and customer roles, and that default stands for a full audit). This is pattern-matching against a clear rubric, not deep architectural reasoning. If the user explicitly requests Opus for a specific role, override that role only; the task-start Model Routing table defers to this skill for the scan.
