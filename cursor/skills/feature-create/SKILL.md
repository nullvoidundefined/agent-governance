...
---
<!-- Cloned from claude/skills/feature-create/SKILL.md. Do not edit here; change the source and re-copy; enforce/tests/skills-lint.test.sh fails when this copy drifts. -->
...
**Stack assumption:** the scaffold script detects the install and test commands from the lockfile and `package.json` (pnpm, npm, or yarn) and skips a scaffold step whose target directory the project does not have (`docs/feature-list/`, `docs/user-stories/`). Override the commands with `FEATURE_CREATE_INSTALL_CMD` and `FEATURE_CREATE_TEST_CMD` (`skip` omits one) for any other stack.
...
|---|---|---|
| 2 | Bad slug or usage | Ask for a lowercase hyphenated slug |
