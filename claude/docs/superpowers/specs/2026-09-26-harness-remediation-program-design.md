...
- 2026-09-26, row 2a (IAN-436), owner decision at row start: build the cheap version at Standard tier instead of the Complex design. Every blocking guard that expands `$HOME` fills it from the account entry, `guard-fail-closed.test.sh` probes every blocking guard with `HOME` unset (dynamic) and checks the fill is present (static), `verification-gate.test.sh` covers the Stop gate, and `harness-sync.sh` installs `jq` remotely or warns without it. No generic crash trap and no per-event failure decision: the survey showed gates allow by exiting silently, so a trap keyed on exit status is the misfire-prone part. `ticket-at-start-gate` keeps its explicit degraded path (IAN-149 test R-6) by owner decision. Row 2b still depends on 2a and is unaffected. Row 4's fail-closed posture, which row 2a was to supply, moves into row 4's own design.
...
- 2026-09-26, row 3d (IAN-441), owner decision at row start: the machine-wide cap is the environment variable `FIXTURE_SHARDS_MAX_RUNS`, like `FIXTURE_SHARDS_LOCK_WAIT_SECONDS`, rather than a runner argument the Stop gate would never pass or a config file, because raising it adds load but cannot skip or shorten a fixture. The worktree lock is keyed on the checkout's top directory, so the enforce and hook trees of one checkout still share a lock; outside any repository the key falls back to the tests directory, so there they do not. "Leave the full suite to CI" needed no code: the Stop gate already runs `--affected`, and IAN-429 narrows what that selects.
