...
| `CLAUDE-FRONTEND-REACT.md` (new) | ~170 | `**/*.tsx`, `**/*.jsx`, `**/src/state/**` (hooks and providers are `.ts` files that need the React rules; `src/state/` is React-only vocabulary, Vue uses `app/composables/` and `app/stores/`) | React 19 Framework and Stack, TSX Component Patterns (props interface, `useCallback`, default exports, no `React.FC`), React Import Ordering, State Management (TanStack Query, Context, `useState`, `useRef`, the state-library ban), `'use client'` pointer to the Next file, JSX Prettier row, React ESLint rules. |
