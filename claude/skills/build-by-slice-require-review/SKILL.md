...
For a hard or risky PR, write a one-paragraph explain-back of what it does and why before merge, and offer a second independent review by a fresh Claude subagent on a stronger model, in addition to the Codex review every PR already gets; offer Copilot only when R-514's Copilot budget allows a round.

## Pre-merge Codex review (every PR, blocking)

Before any PR merges, Codex (OpenAI's coding agent, run through its CLI as a separate process) reviews the PR's diff against the spec and the acceptance criteria of the PR's block in the slice plan document (R-517). It is the review every PR gets, a Copilot round under R-514's budget being an optional extra, and it runs before the user's Gate 2 approval, so the user reviews a PR whose Codex findings are already dispositioned.
