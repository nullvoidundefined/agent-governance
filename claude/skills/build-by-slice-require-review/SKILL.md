...
- **PR body:** a `## Codex review` section carries a `reviewer` line naming the reviewer that ran and why, a `model` line naming the model it ran on, a `range` line naming the diff it read, and one line per finding with its severity and disposition. The range must contain the PR's head commit, so a review that ran before the last push is re-run on the new range rather than re-typed. `git-workflow-guard` denies `gh pr merge` while the section is missing, empty, missing one of the three lines, or naming a stale range.
