...
argument-hint: [owner/repo] [--check] [--stack node|python|go|ruby] [--no-product-docs] [--no-stack-doc] [--no-observability-doc]
...
| `product-docs` | `docs/feature-list/features.md`, `docs/user-stories/README.md`, and `scripts/require-feature-checklist.sh`, written from `~/.claude/prompts/` and `~/.claude/enforce/require-feature-checklist.sh` when absent, never overwritten | R-607: every application repository keeps a features list and per-area user stories from its first commit, so `feature-create` and `task-cleanup` always have somewhere to write, and the repository's own git hook or CI can run the checklist. `SKIPPED` when `.enforce.json` sets `productDocs` to `false` |

| `stack-doc` | `docs/stack.md`, written from `~/.claude/prompts/stack-template.md` when absent, never overwritten | R-608: every application repository lists its stack by layer from its first commit, each entry with version, a plain explanation, the official docs link, its role, why it was chosen, and where it is configured, so each task that changes a dependency has an entry to update. `SKIPPED` when `.enforce.json` sets `stackDoc` to `false` |
| `observability-doc` | `docs/observability.md`, written from `~/.claude/prompts/observability-template.md` when absent, never overwritten | R-608: the catalog of every analytics event, log event, error code, tracker rule, request-ID hop, health endpoint, and metric exists from the first commit. `SKIPPED` when `.enforce.json` sets `observabilityDoc` to `false` |

`--no-product-docs` opts a library or tooling repository out of `product-docs` by recording `"productDocs": false` in `.enforce.json`, which the harness push gate also reads; ask the user before passing it, since an application repository that opts out loses R-607's check entirely. `--no-stack-doc` and `--no-observability-doc` do the same for the two R-608 documents, each recording its own key (`stackDoc`, `observabilityDoc`); the three flags are independent, so a library that passes only `--no-product-docs` still gets both R-608 documents. Ask before passing any of them.
...
4. Commit the written files together, `.claude/settings.json`, `.claude/hooks/harness-bootstrap.sh`, and the product docs included (`chore(repo): CI, Dependabot, PR template, gitignore, harness, product docs baseline`), open the first PR, and confirm the `ci` check appears on it; a ruleset that requires a context no workflow reports blocks every merge until the workflow runs once.
