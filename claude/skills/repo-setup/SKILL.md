...
argument-hint: [owner/repo] [--check] [--stack node|python|go|ruby] [--no-product-docs]
...
`--no-product-docs` opts a library or tooling repository out of `product-docs` by recording `"productDocs": false` in `.enforce.json`, which the harness push gate also reads; ask the user before passing it, since an application repository that opts out loses R-607's check entirely.
