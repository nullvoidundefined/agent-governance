...
R-305: Use the fixed vocabulary in the web client's `src/` (`app`, `components`, `features`, `services`, `api`, `clients`, `state`, `config`, `constants`, `data`, `styles`); context providers live in `state/`; one component per folder (`components/Header/Header.tsx`). A Nuxt client roots at `app/` (`pages`, `layouts`, `middleware`, `plugins`, `components`, `features`, `composables`, `stores`, `api`, `clients`, `services`, `config`, `constants`, `data`, `styles`, `types`) and `server/` (`api`, `routes`, `middleware`, `plugins`); `composables/` and `stores/` replace `state/`; page and Nitro route directories are kebab-case URL segments; `.vue` components pair in folders the same way (`components/TripCard/TripCard.vue`) (CLAUDE-FRONTEND-VUE.md, CLAUDE-FRONTEND-NUXT.md). [hook:structure-gate]
