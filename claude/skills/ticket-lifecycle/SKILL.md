...
Every write here is a `create`, `update`, `save`, `add`, or `comment` MCP call. Since 2026-09-17 (R-105's tracker narrowing), `hooks/mcp-action-guard.sh` stays silent on a plain tracker write and asks only when the call also lands code, submits for review, uploads a file, or applies a change; a denial on one of those asking calls is a decision: do not re-ask for the same write in the same turn, and note in the close report that the ticket is behind the work. When it does ask, one call per confirmation: never batch several writes behind one prompt. Never suppress the guard and never ask to bypass it (R-203).
...
- **Rules:** R-605 (a ticket per task above trivial), R-606 (actuals at close), R-901 (tier), R-903 (model), R-906 (estimate recalibration), R-105 (confirmation on a write that lands code, submits, uploads, or applies; a plain bookkeeping write is exempt), R-106 (nothing client-identifying in this repo)
