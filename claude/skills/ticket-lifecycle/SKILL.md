...
4. Read `started_at` from the `## Session start (R-503)` block that `hooks/session-start.sh` injects at every session start, compaction included, whenever the SessionStart payload carries a transcript path (Claude Code does; the Cursor adapter does not, so under Cursor no block appears). Block summarized away: read the file its `Record:` line names, `session-start.<session-id>` in the session's project directory under the Claude home. Neither present: leave `started_at` empty and say so in the report. Never estimate it, never round it, and never substitute the current time.
