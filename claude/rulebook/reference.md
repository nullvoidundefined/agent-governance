...
R-514: Never merge a PR without explicit user authorization; an owner-approved slice plan is that authorization for its own PRs once CI is green and the R-517 review has passed.
  Spec:
  ...
  - Plan authorization (owner decision 2026-09-23, IAN-333: "Drop the PR ceremony"; measured per-PR overhead ~60 min against ~38 min of implementation, IAN-328): when the owner has approved the slice plan a PR belongs to, merge it on green CI and a passed R-517 review without asking again in chat. The hook's `gh pr merge` prompt still applies. Outside an approved plan, the authorization comes in the current turn.
...
R-517: Before any PR merges, have one reviewer in a fresh context review its diff against the spec and the acceptance criteria, fix or answer every finding in the PR, and summarize the findings and their dispositions in a `## Codex review` section of the PR body that names the reviewer, the model, and a range containing the PR's head commit.
  ...
  - Order: the background bookkeeping subagent (R-605) finishes its file edits, the main session commits them with the slice, and only then does the review run. After the review, change only the PR body, title, or labels, which move no commit. This order is what keeps one review per PR from turning into two.
...
R-518: Open a draft PR as soon as a non-default branch that has never had a PR is pushed, and turn on the desktop app's PR monitor for every PR that opens.
  ...
  - The PR body, which replaced the retired `docs/prs/` document (IAN-333), is written by the background bookkeeping subagent before the draft is marked ready for review (`gh pr ready`): a draft opened by this hook is the start of the review, not a request for it.
...
R-605: Open a tracker ticket for every task above the trivial tier, at classification.
  ...
  - Advance only at `in-progress`, `in-review`, `blocked`, and `done` (owner decision 2026-09-23, IAN-333: "Drop the PR ceremony"; measured per-PR overhead ~60 min against ~38 min of implementation, IAN-328), writing both the status change and a transition comment carrying the UTC ISO-8601 timestamp. `specced` and `planned` stay available and are written only when the owner asks.
  - Background bookkeeping: every ticket write (open, advance, close), the PR body, the R-607 product-doc rows, and the handoff are dispatched to a background subagent on `haiku` or `sonnet` (Agent tool `run_in_background: true`), running beside the implementation; the main session never does them inline and never ends a turn to wait for one. The subagent may edit files but never commits, and the main session commits its edits with the slice before the R-517 review. At open, the main session reads and plans while the subagent works, and it waits for the key only at the first edit, which `ticket-at-start-gate` requires. Declare the product-doc and handoff paths in the task's `--scope` so the subagent's edits do not trip R-212.
...
R-606: Close the ticket with measured actuals, after the verification gate and never before.
  ...
  - Who: the R-605 background subagent writes the close, from the start timestamp, the git log, and the PR the main session names in its prompt; the main session reports the one-line recalibration from the subagent's result.
