...
R-215: Cite a commit in a document only when a ref reaches it.
  ...
  - A citation that is deliberately unreachable, such as a historical note about a branch that was thrown away on purpose, carries the marker `<!-- unreachable-sha: <sha> <reason> -->` on the same line as the citation. The marker must name the same SHA the citation names, must sit on that same line, and must carry a reason after the SHA. All three conditions exist so that the hatch cannot open by accident: a marker copied from another line excuses nothing, a marker left blank excuses nothing, and a marker one line away from its citation excuses nothing.
  - What counts as a citation is a backticked token of 7 to 40 hexadecimal characters in a document under `docs/` or `claude/docs/`. A token that resolves to no commit in this repository is never a finding, because documents legitimately cite another repository's commits and short hexadecimal strings occur in prose; only a token that resolves here and is reachable from no ref is one.
  - `enforce/doc-sha-reachability.sh` is the check. `--changed` (the push boundary's mode) reads the documents this branch touched, `--all` (the default) reads the whole corpus, and a path argument reads exactly that document. Exit 0 is clean, exit 1 names each finding as `DOC-SHA-UNREACHABLE: <sha> at <file>:<line>`, and exit 2 is a state it refuses to judge from, which is a usage error, a directory that is no repository, or a shallow clone whose missing objects would make every citation look foreign.
  - Where it runs. The local pre-push hook runs `--changed`, which is the boundary that matters, because the objects exist in the author's clone and nowhere else: by the time a fresh clone could ask the question, the answer is already lost. The fixture runs in the CI `fixtures` job and at turn end, and carries the IAN-260 spec as a live regression case.
  ...
  Enforcement: ci:doc-sha-reachability (enforce/doc-sha-reachability.sh, run by hooks/pre-push.sample on the changed documents and by enforce/tests/doc-sha-reachability.test.sh in the fixture suites). The check cannot see a citation that is already unreachable in every clone, since the object it would resolve is gone; it catches the citation while the author still holds the object, which is when it can still be pinned.
