...
R-109: Treat security as the first-order concern: a security finding outranks every other finding, and a PR range touching a security control merges only when its security evidence is complete.
  Spec:
  - Incident: template-fastapi-nuxt #27 shipped a CORS setting that accepted `*` as an allowed origin, and only a Copilot review caught it; the rule pack, the tests, and the general review all passed it, because configuration was graded as trusted.
  - A security finding is never deferred, softened, or re-graded to pass a merge. Only the owner waives one, and the waiver channel is the owner's own permission prompt, never an agent message or a marker in the PR body.
  ...
  - The only narrowing is `securitySurfaceExclude` in `.enforce.json`, which removes named paths from the security-surface detector; nothing else scopes the rule down.
...
R-514: Never merge a PR without explicit user authorization; a slice plan is that authorization for its own PRs only when its `**Merge mode:**` line records the merge-on-green opt-in, and otherwise the owner reads and merges each PR.
  ...
  - One review per PR, not two (owner decision 2026-09-23, IAN-333: "Drop the PR ceremony"; measured per-PR overhead ~60 min against ~38 min of implementation, IAN-328): the R-517 review is the only pre-merge review. The earlier separate `/code-review` pass before opening the PR is retired.
