...
R-334: Name every schema, model, type, module, and store with its base noun first and its secondary nouns after, so a name states what it belongs to before it states what it is.
  ...
  - The forms, by layer: table `{root}_{entity}s`; foreign key column `{root}_{entity}_id`; enum type `{root}_{entity}_{attribute}`; model class the PascalCase compound; schema class the model plus its role (`TripOfferCreate`, `TripOfferResponse`); repository module the table name; repository function a verb plus the compound noun (`load_trip_offer`, `list_trip_offers_by_trip`); frontend store and component the same compound.
  - Two exceptions, each documented where it occurs. An aggregate root is named for itself and takes no prefix. A table a third-party framework creates and writes keeps that framework's names, because renaming it breaks the framework.
  ...
  Enforcement: judge (the push-time semantic judge reads this Spec and the diff; it decides whether a new table, model, type, module, or store name carries its root). A deterministic gate needs the per-project root list the `domain-lexicon` skill produces; until that skill exists the judge is the only enforcer, and this Spec says so rather than implying a check that does not run.
