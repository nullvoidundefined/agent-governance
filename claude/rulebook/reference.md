...
R-514: Never merge a PR without explicit user authorization in the current turn.
  Spec:
  - Claude may create PRs and push branches, and may request a Copilot review only under the Copilot budget below.
  - Default path: (1) CI passes; (2) the R-517 review has run and its findings are fixed or answered (a trivial-tier PR is exempt and merges on green CI under the same merge authorization as any PR), and the Copilot round passes when the budget allowed one; (3) the user explicitly asks to merge after CI and every review in (2) are confirmed green, or a project rule grants standing merge-on-green authorization. The trivial path removes the review wait, never the authorization or the `gh pr merge` prompt. "Merge when ready" is not authorization.
  ...
  - Before merge, the blocking Codex review of R-517 has run on the final behavior-changing range and the PR body carries its `## Codex review` section (a ledger-verified trivial-tier PR is exempt, per R-517's Scope); the R-517 review is the review every PR above trivial gets, and a Copilot round is an extra outside opinion, never a substitute for it.
  - Copilot budget (owner decision 2026-09-20, IAN-163; the monthly limit is $20): request Copilot only on a Complex or Saga PR, one round per PR, and never re-request it after fixes. Before requesting, read this month's Copilot spend with `gh api "/users/$(gh api user -q .login)/settings/billing/usage?year=$(date +%Y)&month=$(date +%-m)" --jq '[.usageItems[] | select(.product=="copilot") | .netAmount] | add // 0'` (every Copilot SKU counts, code review and the cloud agent alike) and the rounds already spent with `gh api -X GET search/issues -f q="is:pr reviewed-by:copilot-pull-request-reviewer[bot] created:>=$(date +%Y-%m)-01 user:$(gh api user -q .login)" -q .total_count`; request no round, and say so in the PR, when the spend is $15 or more or six rounds have run. Keep every repository ruleset that auto-requests Copilot review disabled (in agent-governance, `copilot-review-main-and-slice`). After a fix, re-run the R-517 review on the new range instead of Copilot.
