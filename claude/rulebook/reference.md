...
R-334: Name every schema, model, type, module, and store with its base noun first and its secondary nouns after, so a name states what it belongs to before it states what it is.
  ...
  - The forms, by layer: table `{root}_{entity}s`; enum type `{root}_{entity}_{attribute}`; model class the PascalCase compound; schema class the model plus its role (`TripOfferCreate`, `TripOfferResponse`); repository module the table name; repository function a verb plus the compound noun (`load_trip_offer`, `list_trip_offers_by_trip`); frontend store and component the same compound. These forms are written in `snake_case` because the reference stack is PostgreSQL and Python; under a `camelCase` engine or language the same words appear in the same order with the underscore dropped (`{root}{Entity}s`, `{root}{Entity}{Attribute}`).
  - Foreign keys and document reference fields are not a separate form. A reference is the referenced entity's singular plus the engine's id suffix, `{referenced_table_singular}_id` under a `snake_case` engine, exactly as `CLAUDE-DATABASE.md` defines it today, and `{referencedEntitySingular}Id` under a `camelCase` one, which the document-store track defines when it lands, and this rule changes nothing about either: because the referenced entity is itself compound, its singular already carries the root, so `trip_legs` yields `trip_leg_id` and `tripLegs` yields `tripLegId` without a second rule saying so. Stating the form twice in two places is how one key acquires two names.
...
R-361: Never query once per element of a collection (the N+1); load or write the set in one statement.
  ...
  - Write a set in one statement: `INSERT ... SELECT * FROM unnest($1::uuid[], $2::text[])`, `UPDATE ... FROM unnest(...)`, `DELETE ... WHERE id = ANY($1::uuid[])`.
  - A repository offers the plural form (`listJobsByIds(jobIds)`, `listLegsByTripIds(tripIds)`) as soon as any caller needs more than one row; a caller that needs many never loops over the singular.
  - A loop that repeats a query on purpose, with a stated bound (keyset batching in a worker, one query per 1000 rows), carries `// eslint-disable-next-line dataAccess/no-query-in-loop -- <the bound>`.
...
R-363: Make every read-modify-write atomic in the database; never read a value, change it in application code, and write it back unguarded.
  ...
  - "Select, then insert when absent" is a race; a unique constraint with `ON CONFLICT` decides it.
  - An idempotent endpoint or webhook handler records its idempotency key under a unique constraint in the same transaction as the effect.
...
R-364: Bound every read by the request, never by the size of the table.
  ...
  - Every `ORDER BY` ends in a unique column (`id`) so a page boundary is deterministic.
  - Aggregate in SQL (`COUNT`, `SUM`, `GROUP BY`), never by loading rows to count or sum them in memory; test existence with `EXISTS` or `LIMIT 1`, never `COUNT(*) > 0`.
  - Select only the columns the caller uses when the table carries large columns (`jsonb` payloads, long `text`) the endpoint does not return; `SELECT *` stays the default for single-row reads and narrow tables.
  - Every column in a `WHERE`, `JOIN`, or `ORDER BY` is covered by an index, and a new query against a table that can grow past a few thousand rows is checked with `EXPLAIN ANALYZE` on realistic data before merge.
...
R-365: Pass only values as query parameters and put only allowlisted identifiers into SQL text; never build SQL from caller input.
  ...
  Enforcement: judge (warn; decides a value interpolated into SQL text and an identifier taken from caller input without an allowlist)
...
R-406: Give every user-input handler one negative-input test, and every security control a test that feeds it its insecure value.
  ...
  - What counts as a security control is the security-surface detector's definition (`hooks/security-surface.sh`), so the set this rule covers matches the set R-109's merge evidence is required for.
  Enforcement: manual
...
R-514: Never merge a PR without explicit user authorization; a slice plan is that authorization for its own PRs only when its `**Merge mode:**` line records the merge-on-green opt-in, and otherwise the owner reads and merges each PR.
  Spec:
  - Claude may create PRs and push branches.
  - Default path: (1) CI passes; (2) the R-517 review has run and its findings are fixed or answered (a trivial-tier PR is exempt; as the one standing exception to this path, a trivial-tier PR merges on green CI without per-PR authorization when the task-tier ledger records the trivial tier for its head branch, owner decision 2026-09-26, IAN-433); (3) the user explicitly asks to merge after CI and every review in (2) are confirmed green, or a standing authorization already covers this PR, which since IAN-352 means a slice plan whose `**Merge mode:**` line records the merge-on-green opt-in (Plan authorization below), or a project rule granting one on those same conditional terms; no project rule grants an unconditional standing merge-on-green, and the one in `personal/.claude/CLAUDE.md` was narrowed to the plan's recorded mode on 2026-09-24. The trivial path removes the review wait, never the authorization or the `gh pr merge` prompt. "Merge when ready" is not authorization.
  - Plan authorization (owner decision 2026-09-23, IAN-333: "Drop the PR ceremony"; measured per-PR overhead ~60 min against ~38 min of implementation, IAN-328; narrowed to an opt-in by owner decision 2026-09-24, IAN-352): approving a slice plan is merge authorization for that plan's PRs only when the plan's `**Merge mode:**` line records the merge-on-green mode, which `build-by-slice-require-review` asks for once per slice at Gate 1 with the owner-merges mode listed first as the default. Under that recorded opt-in, merge each of the plan's PRs on green CI and a passed R-517 review without asking again in chat. Under the default, or under a plan that records no mode at all, hand each PR to the owner with its review findings and their dispositions and let them merge it. IAN-333 had made that owner merge an opt-out rather than the choice it is meant to be, which removed the owner from the loop by default in a skill named require-review, and a subagent review is not a substitute for the owner's own read of the diff. The hook's `gh pr merge` prompt still applies to either mode. Outside an approved plan, the authorization comes in the current turn.
  - One review per PR, not two (owner decision 2026-09-23, IAN-333: "Drop the PR ceremony"; measured per-PR overhead ~60 min against ~38 min of implementation, IAN-328): the R-517 review is the only pre-merge review, except that a security-touching range also requires the R-109 security review. The earlier separate `/code-review` pass before opening the PR is retired.
  - Before merge, the blocking review of R-517 has run on the range ending at the PR's current head and the PR body carries its `## Codex review` section with the `reviewer`, `model`, and `range` lines R-517 requires, the range's head endpoint being that head commit (a ledger-verified trivial-tier PR is exempt, per R-517's Scope); the R-517 review is the review every PR above trivial gets.
  - Never request Copilot review, on any PR, and never add a Copilot reviewer (owner decision 2026-09-20, IAN-163). Keep every repository ruleset that auto-requests Copilot review disabled (in agent-governance, `copilot-review-main-and-slice`). CI, the R-517 review, and on a security-touching range the R-109 security review are the checks before merge; after any commit that moves the head, re-run the R-517 review on the new range (R-517's Timing bullet says why no commit is exempt).
...
R-517: Before any PR merges, have one reviewer in a fresh context review its diff against the spec and the acceptance criteria, fix or answer every finding in the PR, and summarize the findings and their dispositions in a `## Codex review` section of the PR body that names the reviewer, the model, and a range containing the PR's head commit.
  ...
  - Section: the PR body carries a Markdown heading starting with `Codex review` (for example `## Codex review`), and under it a `reviewer` line naming the reviewer that ran, a `model` line naming the model it ran on, a `range` line naming the diff it read, and one line per finding with its severity and disposition, or "No findings" with the areas checked. Each of the three is written as `<label>: <value>` on its own line, optionally bulleted and optionally emphasised, so `- **Reviewer:** Claude subagent (fable)` and `Reviewer: Codex` both read as the same line; a label with no value after the colon counts as absent, because a blank label records nothing. The three lines are what turns the section from a heading anyone can type into an artefact that says who reviewed what, which is the predicate the gate reads.
  - Range, and what a stale one means (owner decision, IAN-286, 2026-09-23): the `range` line must hold a `<base>..<head>` or `<base>...<head>` expression whose head endpoint is the PR's head commit, and any other line denies the merge. A review whose range stops short of the head read a tree other than the one that would merge, so it records that a review happened at some point without saying anything about what is about to land; that is a record of a review rather than the satisfaction of this rule, and accepting it would restore the defect the rule was rewritten to remove. The case that settled it is PR #106, whose review returned findings against `c20e5a8` while the branch had already moved to `8183e6b`, two of its HIGH findings having been fixed before it reported, so its own preamble had to say that it was describing a tree that no longer existed. The remedy for a stale range is to re-run the review on the current `<base>..<head>` and rewrite the line from that run, never to retype the line by hand. The check is on the expression's head endpoint and never on the line as a whole. An earlier draft of this gate accepted any hexadecimal run of at least seven characters anywhere on the line, which passed for the head commit pasted into prose or into a link, and, worse, for `<head>..<later>`, a range whose base is the head and which therefore reviewed everything except it. That reduced the artefact to pasting the object name somewhere on the line, which is a longer ritual rather than evidence, so the line is now parsed: the first whitespace-separated token holding `..` is the range, its base must be non-empty, and its head endpoint, after trailing punctuation is removed, must be hexadecimal, at least seven characters (git's own abbreviation length, and shorter runs collide with ordinary words), and a prefix of the head commit's object name. The comparison is textual rather than a `git merge-base` call, because the head commit of a pull request need not exist in the checkout the merge runs from and an ancestry question asked there would answer "no" for a range that is in fact current.
  - Codex fallback: when an opted-in Codex review cannot run (unavailable, unauthenticated, or out of quota), do not wait for the quota and do not review in the main session; dispatch the default subagent reviewer, on `opus` for a diff that justified Codex, with the same prompt, and name the fallback on the `reviewer` line.
...
R-518: Open a draft PR as soon as a non-default branch that has never had a PR is pushed, and turn on the desktop app's PR monitor for every PR that opens.
  ...
  - The PR body, which replaced the retired `docs/prs/` document (IAN-333), is written before the draft is marked ready for review (`gh pr ready`), by the main session, or on a Complex or Saga task by the background bookkeeping subagent (R-605): a draft opened by this hook is the start of the review, not a request for it.
