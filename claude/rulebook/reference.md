...
R-514: Never merge a PR without explicit user authorization in the current turn.
  ...
  - Before merge, the blocking Codex review of R-517 has run on the final behavior-changing range and the PR body carries its `## Codex review` section with the `reviewer`, `model`, and `range` lines R-517 requires, the range containing the PR's head commit (a ledger-verified trivial-tier PR is exempt, per R-517's Scope); the R-517 review is the review every PR above trivial gets.
...
R-517: Before any PR merges, have Codex review its diff against the spec and the acceptance criteria, fix or answer every finding in the PR, and summarize the findings and their dispositions in a `## Codex review` section of the PR body that names the reviewer, the model, and a range containing the PR's head commit.
  ...
  - Range, and what a stale one means (owner decision, IAN-286, 2026-09-23): the `range` line must contain the PR's head commit, and a range that does not denies the merge. A review whose range stops short of the head read a tree other than the one that would merge, so it records that a review happened at some point without saying anything about what is about to land; that is a record of a review rather than the satisfaction of this rule, and accepting it would restore the defect the rule was rewritten to remove. The case that settled it is PR #106, whose review returned findings against `c20e5a8` while the branch had already moved to `8183e6b`, two of its HIGH findings having been fixed before it reported, so its own preamble had to say that it was describing a tree that no longer existed. The remedy for a stale range is to re-run the review on the current `<base>..<head>` and rewrite the line from that run, never to retype the line by hand. The containment check is textual rather than a `git merge-base` call, because the head commit of a pull request need not exist in the checkout the merge runs from and an ancestry question asked there would answer "no" for a range that is in fact current: every hexadecimal run of at least seven characters in the line is read as an object name, and one of them must be a prefix of the head commit's, so any abbreviation from seven characters up to the full name is accepted and six or fewer is too ambiguous to accept.
