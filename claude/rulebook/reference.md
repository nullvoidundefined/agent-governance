...
R-514: Never merge a PR without explicit user authorization in the current turn.
  ...
  - Before merge, the blocking Codex review of R-517 has run on the final behavior-changing range and the PR body carries its `## Codex review` section (a ledger-verified trivial-tier PR is exempt, per R-517's Scope); Copilot and Codex are two independent reviews, and neither substitutes for the other.
