...
R-514: Never merge a PR without explicit user authorization in the current turn.
  ...
  - Copilot budget (owner decision 2026-09-20, IAN-163): Copilot code review bills per AI credit, credits scale with diff size, and the monthly allowance runs out, so request it sparingly. Request Copilot only on a Complex or Saga PR, one round per PR, and never re-request it after fixes. Before requesting, sum this month's Copilot `netAmount` from `gh api "/users/$(gh api user -q .login)/settings/billing/usage?year=<YYYY>&month=<M>"` and count the Copilot rounds already requested this calendar month; skip the round and say so in the PR when the sum is $15 or more or six rounds have run. Keep the repository ruleset that auto-requested Copilot on every PR (`copilot-review-main-and-slice`) disabled. After a fix, re-run the R-517 review on the new range instead of Copilot.
