...
R-105: Obtain explicit confirmation before any destructive MCP action (delete, drop, rotate, send, post, create) unless pre-authorized this turn.
  ...
  - The active tracker's own tools are pre-authorized: a tool name listed under `trackers.<active>.tools` in `~/.claude/TICKET-TRACKER.json` passes without a prompt, because the ticket-lifecycle skill writes through exactly those tools and the user chose the tracker. An absent or malformed config pre-authorizes nothing. Every other MCP write, on the same server or any other, still asks (2026-09-17, owner decision). The config file itself is a protected path: any Write, Edit, or Bash write targeting it asks through protected-path-guard, so a session cannot widen the standing authorization silently.
  Enforcement: hook:mcp-action-guard (asks; "don't ask again" on a specific tool is the user's own pre-authorization; the tracker config's tool list is the standing one)
