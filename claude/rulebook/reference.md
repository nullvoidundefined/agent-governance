...
R-503: Announce each task's percentage share of total work and capture a start timestamp for any multi-step project.
  ...
  Spec:
  - Session start: `hooks/session-start.sh` records the start timestamp, UTC ISO-8601, write-once to `~/.claude/projects/<key>/session-start.<session-id>` and injects it as a `## Session start (R-503)` block on every start, compaction included. The value is the first `timestamp` in the session transcript, or the hook's own clock when no transcript is on disk yet. Read it from there; never recall or estimate it. ticket-lifecycle's `open` takes `started_at` from it.
