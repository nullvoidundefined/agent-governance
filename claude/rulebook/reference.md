...
R-512: Squash-merge feature branches: `git merge --squash`; one commit per feature on `main`.
  ...
  - Bundle exception (IAN-122, 2026-09-19): 2 to 5 small, related tickets may ship as one PR labeled `bundle`, so they share one CI run and one Copilot pass. The branch carries exactly one commit per ticket; each commit has a conventional subject and its own `Refs: <KEY>` trailer line. Merge it with `gh pr merge <n> --rebase` so every ticket keeps exactly one commit on `main`. Squash the ticket's fixups into its commit before the merge (`git rebase` with `fixup!` commits and `--autosquash`), never leave a review fix as its own commit.
...
R-514: Never merge a PR without explicit user authorization in the current turn.
  ...
  - Default path: (1) CI passes; (2) Copilot review passes, except for a trivial-tier PR, which requests no Copilot review (IAN-98, 2026-09-18) and merges on green CI under the same merge authorization as any PR; (3) the user explicitly asks to merge after both are confirmed green, or a project rule grants standing merge-on-green authorization. The trivial path removes the review wait, never the authorization or the `gh pr merge` prompt. "Merge when ready" is not authorization.
  ...
  - Before merge, the blocking Codex review of R-517 has run on the final behavior-changing range and the PR body carries its `## Codex review` section; Copilot and Codex are two independent reviews, and neither substitutes for the other.
  - Request a second Copilot round only when round one led to a behavior change (code or tests). Fixes to wording, docs, or the PR description merge without a re-review.
  - After opening a PR and requesting review, start the next ticket and return when CI and Copilot finish; never block the session on polls.
...
R-517: Before any PR merges, have Codex review its diff against the spec and the acceptance criteria, fix or answer every finding in the PR, and summarize the findings and their dispositions in a `## Codex review` section of the PR body.
  Scope: every PR in every tier, trivial included (owner decision, 2026-09-19: "Codex reviews every PR before merge, blocking"). It runs alongside the Copilot review of R-514, never instead of it, and alongside `/code-review` and `spec-conformance-review`.
  Spec:
  ...
  Enforcement: hook:git-workflow-guard (denies every `gh pr merge` whose PR body, read with the same `gh pr view --json labels,commits,body` call R-512 uses, lacks a heading starting with "Codex review" followed by at least one non-blank line before the next heading, ignoring fenced code blocks; fails closed when `gh` errors, answers with anything that does not parse, or exceeds `CLAUDE_GH_TIMEOUT_SECONDS`, when the command runs `cd`/`pushd` or sets `GH_REPO`/`GH_HOST`, when one command runs more than one merge, and on a recognized merge shape the parser cannot read: `gh` escaped, quoted, named by path, or behind a common wrapper (`env`, `command`, `exec`, `sudo`, `nohup`, `time`, `timeout`, `nice`, `xargs`, `eval`, `bash -c`), or an option such as `--repo`/`-R` before the `merge` subcommand; unescaped backslash-newline continuations are joined before matching; a heading inside fenced code, indented code, or an HTML comment does not count). Known limit: the wrapper list is a denylist and cannot be complete, and a subshell `(gh pr merge ...)` is deliberately not matched, as it never was for R-512 and R-514, so a parenthesized mention in a commit message is not read as a merge. Running the review, the quality of each disposition, and the re-run after a behavior change are manual; the hook proves only that the section exists and is not empty.
