...
R-907: Write implementation code and the tests that verify it with a different author than the one that wrote the code; never let the author of the implementation also write its own tests.
  ...
  Degradation: when the codex CLI is missing, unauthenticated, or rate-limited, do not stall the slice: dispatch the `test-author` agent (Complex/Saga) or confirm the guard's ask (Standard, inline), and record `skipped: challenger unavailable (<reason>)` in the session handoff so the gap is visible rather than silent.
