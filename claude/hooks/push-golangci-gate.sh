...
if [ -n "$REPORT" ]; then
  ...
  jq -n --arg r "golangci-lint enforcement failed on the outgoing diff (R-324/R-329/R-344 Go analogs; mapping in enforce/golangci-enforce.yml). Fix the violations:
