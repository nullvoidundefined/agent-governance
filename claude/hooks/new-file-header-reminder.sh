...
# TypeScript, a module docstring in Python, a leading HTML or script comment in
# a Vue component). A hook cannot author a good header,
...
jq -rc '
  ...
  | if ($p | test("\\.(ts|tsx|js|jsx|mjs|cjs|py|vue)$"))
    ...
    then {hookSpecificOutput:{hookEventName:"PostToolUse",additionalContext:("New file "+$p+" has no file-level header. Per the what+why convention, add a header at the very top stating what the file does and why (a /** ... */ block in TypeScript, a module docstring in Python, an HTML comment or a leading script comment in a .vue file), unless it is genuinely self-explanatory (barrel, single-constant, or pure type re-export). No em dashes.")}}
    else empty end
' 2>/dev/null || true
