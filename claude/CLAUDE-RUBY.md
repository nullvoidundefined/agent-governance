...
- The same gate runs `~/.claude/enforce/data-access/ruby_data_access.rb` (stdlib Ripper) over the changed Ruby files, added lines only: R-361, an ActiveRecord finder or writer on a model constant, connection-level SQL (`exec_query`, `select_all`), or a query object's `.call` inside an iteration block or a `while`/`until`/`for`; R-362, inside a `transaction` or `with_lock` block, an HTTP client call, a mailer `deliver_*`, or a job `perform_later`/`perform_async` (an `after_commit` block is the fix and passes). Lazy association access in a loop is not statically decidable: use `includes` or `preload` and `strict_loading`. The checker runs even without RuboCop and fails open without `ruby`. A deliberately bounded loop carries `# data-access-allow: <the bound>` on the line or the line above.
