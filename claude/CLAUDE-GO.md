...
- The same gate builds and runs `~/.claude/enforce/data-access/go/main.go` (stdlib `go/ast`) over the changed `.go` files, added lines only: R-361, a `Query`/`QueryRow`/`Exec`/`CopyFrom` call, a `repositories` package call, or a `*Repository`/`*Repo` receiver call inside a `for` body, clause, or a goroutine or `errgroup` literal there (`SendBatch` is the fix and passes); R-362, inside a `BeginFunc`/`BeginTxFunc`/`WithTx`-style callback with a `Tx` parameter, a data-access call not on or given the tx, or a `net/http` or `clients` call. It parses and never compiles the target, so it runs in repos off the trust list too; it fails open without `go`. A deliberately bounded loop carries `// data-access-allow: <the bound>` on the line or the line above.
