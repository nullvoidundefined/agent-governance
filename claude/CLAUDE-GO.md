...
- Test runs (R-509): `go test` already runs packages in parallel; mark independent tests `t.Parallel()` and keep them free of shared globals so they can run in parallel. Turn ends, commits, and branch-level merges test only the packages containing changed files and the packages that import them (select with `go list -f '{{.ImportPath}} {{join .Deps " "}}' ./...`); the full `go test ./...` runs as the required CI check before any merge to `main`, not at pre-push (IAN-98).
