...
6. Run a dry test: `bash enforce/doctor.sh --full` (wraps both fixture suites, 70 fixture tests total, plus the install checks: settings parse, schema keys, hook registration, hook integrity, hook executability, deps, sandbox availability, statusline, port freshness; see `enforce/README.md`), then start a session and confirm the `SessionStart` hook emits the global memory INDEX. Try a Write call containing U+2014 and confirm it blocks.
