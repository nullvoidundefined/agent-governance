# claude: the Claude Code surface of agent-governance
...
│   ├── eslint-options.mjs            # Options shared by lint.mjs and ratchet.mjs.
...
│   └── tests/                       # fixture tests; run-tests.sh runs all in parallel, `# Shard: serial` ones alone after; --affected runs every fast fixture plus the slow or serial ones a change names or watches.
...
│   ├── repo-setup/                  # Brings a GitHub repo to the hygiene baseline: CI, Dependabot,
│   │                                # Greptile, protected main/staging, squash-only merges, alerts.
...
│                                    # root docs/, read from each project's git toplevel.)
└── projects/, sessions/, cache/, ... (runtime state; not tracked by git)
```

## How a session uses this repo

1. **Session start.** Claude Code loads `~/.claude/settings.json`, which wires the `SessionStart` hooks. `harness-sync.sh` runs first (R-003): it compares the live `~/.claude` against the checkout named in `.sync-source` (or the project directory when that is itself an agent-governance clone) and runs `./sync.sh` when any tracked file differs, installing `rsync` in a cloud container when it is missing; this repository's own `.claude/settings.json` registers the same hook so a cloud session on this repo bootstraps its harness before anything else loads, and the `repo-setup` skill installs a bootstrap hook that clones and syncs the checkout into any other repository. `session-start.sh` reads `global-memory/INDEX.md` and the project's `docs/session-handoff/session-handoff.md` (SHA-verified against git log before it is trusted) and emits both as additional context, together with the session's start timestamp (R-503), taken from the transcript's first `timestamp` and recorded once per session so ticket-lifecycle reads `started_at` rather than recalling it. `hook-integrity-check.sh` verifies the enforcement scripts against the committed hash manifest; `enforcement-guard-check.sh` verifies the manifest/settings/rulebook closure and warns if the llm-judge tier cannot run.
2. **Rules load.** `CLAUDE.md` is loaded into the session: one norm line per rule with its enforcer named inline, currently 77 rules in 125 lines. The 200-line cap is not a convention but a test: `enforce/tests/claude-md-lint.test.sh` fails the suite above it, and also fails on any rule id that has a Spec in `rulebook/reference.md` but no norm line in either `CLAUDE.md` or a skill (and vice versa), so the two cannot drift apart. The full Spec for every rule lives in `rulebook/reference.md`, read on demand and consumed mechanically by the enforcement guard and the CI rule judge.
...
Six push-time gates guard `git push`: ESLint (TS), ruff (Python), RuboCop (Ruby), golangci-lint (Go), the R-109 security rule pack (`push-semgrep-gate`, which runs the Semgrep rules in `enforce/semgrep/` over the pushed HEAD content of every whole code file the outgoing range changes, exported to a temporary directory so an uncommitted fix cannot change the outcome, with `# nosemgrep` suppressions disabled, denies on any finding with its `path:line rule`, and fails closed, denying the push, when the outgoing base cannot be resolved (`CLAUDE_ENFORCE_BASE` overrides it), when Semgrep is absent, crashes, prints no readable JSON report, or reports any error or skipped path, and when a Python file fails `ast.parse` or a JavaScript file fails `node --check`, because Semgrep can silently drop a finding from a file that does not parse; when no `semgrep` is installed, the `uvx semgrep` fallback downloads Semgrep from PyPI on first use, and the gate runs Semgrep with its version check and metrics disabled), and the R-607 feature checklist (`push-feature-docs-gate`), which refuses a branch that adds a Next, Nuxt, FastAPI, or Express route without also changing `docs/feature-list/features.md`, a story under `docs/user-stories/`, and an e2e spec, and the R-608 stack and observability check in the same hook, which refuses a branch that adds or removes a dependency in a `package.json`, `pyproject.toml`, `Gemfile`, or `go.mod` without changing `docs/stack.md`, or adds or removes an analytics event registry entry, an error-code registry entry, or a log event name without changing `docs/observability.md` (`"stackDoc": false` and `"observabilityDoc": false` in the repository's enforce config opt out of each half). One pull-request gate guards `gh pr create`: `pr-ticket-ref-gate` (R-605) refuses a pull request when neither its commits nor its `--body`/`--body-file` text carry a `Refs: <KEY>` line, exempts docs-only diffs and a trivial tier recorded for the branch, and allows with a warning when `~/.claude/TICKET-TRACKER.json` is absent. The same rule is enforced earlier by `ticket-at-start-gate` (R-605, PreToolUse on Write, Edit, and `git commit`), which refuses the first edit and every commit until task-start's `.claude/task-tier.json` names the branch and carries its ticket key (`task-tier.sh set <tier> "<reason>" --ticket <KEY>`) or the trivial tier, so a ticket exists before the work rather than after the merge. Two PostToolUse hooks follow the pull request (R-518): `draft-pr-on-first-push` opens a draft with `gh pr create --draft` after a successful push of a non-default branch that has never had a PR (a merged or closed one gets a one-line note instead), under the same R-605 checks (shared through `hooks/pr-range-checks.sh`), and never fails the push; `pr-monitor-reminder` tells the session, after any successful `gh pr create`, to turn on the desktop app's PR monitor (`mcp__ccd_pr__set_monitor` with auto-fix, comment handling, and auto-archive) and never to enable auto-merge unasked. A repository opts out of the draft with `"autoDraftPr": false` in `.enforce.json`. The linters fail open when their tool is unavailable, while the Semgrep security gate fails closed; all six honor `enforce/exempt-repos.txt` (origin URL per line). Two trust boundaries are explicit: linters resolve from PATH or the harness bundle, never from the target repo (`bundle exec` is banned; the Go gate additionally requires the repo in `enforce/gate-trusted-repos.txt` because linting Go compiles the tree, and the security gate runs a repo's own `.venv/bin/python3` for its parse check only when the repo is listed there). **LLM rule judge:** the semantic naming rules are judged in CI, not at push: `.github/workflows/rule-judge.yml` runs `enforce/judge-diff.sh` over each pull request's diff, fails the `rule-judge` check on an error-severity finding, and annotates warnings. It left the push path on 2026-09-18 (IAN-98), where it averaged 171 seconds per push. **Egress:** the workflow sends each pull request's diff to `api.anthropic.com` under the repository secret `ANTHROPIC_API_KEY`; without the secret it passes with a notice. Another repository reuses it through `workflow_call`. A git `pre-push` hook runs the Cursor and Codex port checks before any push of this repo (installed from the tracked `hooks/pre-push.sample` by `hooks/install-git-hooks.sh`); the full fixture suites run in CI, not at pre-push. `hooks/verification-gate.sh` blocks a turn, and since 2026-09-06 a writing subagent's return, from ending on a red suite (R-509), and `.github/workflows/enforce.yml` runs both suites plus the ratchet in CI, where `--no-verify` cannot reach them. Inside a session, `destructive-command-guard` also denies the common ways the agent could skip git hooks. It parses each command the way bash does (through `hooks/shell-command-segments.py`, so quoting, escapes, wrappers such as `sudo` and `bash -c`, and compound commands do not hide a git call) and denies `--no-verify` in any spelling git accepts, `commit -n` and `am -n`, the hook-manager variables `HUSKY=0`, `SKIP`, and `LEFTHOOK=0`, `core.hooksPath` overrides and writes, config includes and hook-skipping aliases, a shell function or hook-skipping alias, writes to git config files, and deleting, disabling, or overwriting files under `.git/hooks`; when its parser is missing or fails, it denies any command that names git. It is a guard against habit and accident, not a sandbox: a shell can always reach git in a way no text check sees (a variable holding the flag, a script file), and a human at the terminal can still skip hooks, which is why CI remains the gate that decides.
...
1. **[`CLAUDE.md`](./CLAUDE.md)**: the canonical rules file, one norm line per rule with its enforcer in brackets. Start here and read it whole; at 119 lines that is the cheapest orientation available. Full Specs live in [`rulebook/reference.md`](./rulebook/reference.md), read on demand.
...
6. **`docs/session-handoff/session-handoff.md` at the monorepo root**: the most recent handoff, and the cheapest way to understand current state; the hooks resolve it from each project's git toplevel, so it does not ship into `~/.claude`. Dated audit reports live beside it in the monorepo root `docs/audits/`.

## Running this repo on your own machine

This directory is the `claude/` section of the agent-governance monorepo; `./sync.sh` at the repo root copies it verbatim into `~/.claude/` (`SETUP.md` carries the full bootstrap). To bootstrap:

1. Clone agent-governance anywhere and run `./sync.sh` from the repo root to install `~/.claude/` (plus the `~/.codex` and `~/.cursor` ports).
2. Ensure Claude Code is installed.
3. Verify `jq`, `node`, and `python3` are available (`brew install jq node` on macOS); the hooks depend on all three. Then `npm ci --prefix enforce` (locked, never `npm install`, which can resolve differently from the committed lockfile): `enforce/node_modules` is gitignored, so a fresh clone has none and the six ESLint-backed fixtures fail on a missing ESLint rather than on a real defect.
...
## The Cursor and Codex ports

The consolidation shipped on 2026-09-12: `cursor/` and `codex/` sit beside `claude/` in the agent-governance monorepo, mirroring the rules into Cursor's `.mdc` format and Codex's `AGENTS.md` conventions, and `sync.sh` copies each section verbatim to the fixed path its tool expects (`~/.claude`, `~/.cursor`, `~/.codex`). The old per-target `build.mjs` pipeline and its separate remotes are retired; the surviving `PORT-STATUS.md` and `.claude-port.json` artifacts are frozen at the last build and tracked in `ISSUES.md`. The design rationale lives in `docs/superpowers/specs/2026-09-12-agent-governance-monorepo-design.md`.
...
*Last updated: 2026-09-17. Maintained by the `SessionStart` / `SessionEnd` hooks and by dated commits to `main`.*
