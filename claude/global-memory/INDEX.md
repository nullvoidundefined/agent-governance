...
- [`feedback_no_fluff.md`](./feedback_no_fluff.md): Ban all LLM filler patterns per R-209.
...
- [`feedback_ask_judgment_calls.md`](./feedback_ask_judgment_calls.md): **Canonical** R-211 detail. Two or more judgment calls in a task: ask them with option tiles, one question per turn, never batched and never decided silently.
- [`feedback_card_whenever_waiting.md`](./feedback_card_whenever_waiting.md): Any turn that ends waiting on the user (approval, review, decision, information) ends with an AskUserQuestion card, never a prose ask.
- [`feedback_ask_auto_merge_per_pr.md`](./feedback_ask_auto_merge_per_pr.md): After opening any PR, ask with a card whether to enable auto-merge for it; never enable it unasked, never skip the question.
...
- [`lesson_strip_git_env_in_hook_tests.md`](./lesson_strip_git_env_in_hook_tests.md): Tests that spawn git in a temp repo must strip GIT_* from the child env; under a hook, GIT_DIR leaks and hits the real repo (broke every doppelscript worktree 2026-09-26)
...
- [`feedback_default_sonnet_proactive_switch.md`](./feedback_default_sonnet_proactive_switch.md): Session-default cost discipline. `settings.json` runs `opusplan` (Opus in plan mode, Sonnet for execution; decided 2026-09-05). When work drifts into mechanical territory, surface the mismatch and ask the user to `/model sonnet`; only the user can switch the main session. Per-task routing stays with the canonical `feedback_model_routing.md` entry above.
