...
R-514: Never merge a PR without explicit user authorization; an owner-approved slice plan authorizes merging its PRs on green CI and a passed R-517 review, and outside such a plan the authorization comes in the current turn ("merge when ready" is not authorization); direct pushes to `main` only on express request after naming the risks; never request Copilot review. [hook:git-workflow-guard]
