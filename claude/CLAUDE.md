...
R-605: Open a tracker ticket (`/ticket-lifecycle`) for every task above the trivial tier at classification, carrying title, tier, assist, model, estimate, repo, and branch; search by branch before creating so one branch never gets two tickets; advance it only at `in-progress`, `in-review`, `blocked`, `done`, and `dropped`, with a timestamped transition comment; dispatch every ticket write, the PR body, product-doc rows, and the handoff to a background subagent on `haiku` or `sonnet` running beside the implementation, never inline in the main session, and let that subagent edit files but never commit; keep the key on the spec, the user story, the handoff, and every commit's `Refs:` trailer; a prompt handing work above the trivial tier to a subagent or spawned task names its ticket key or says to open one; record the key with `task-tier.sh set <tier> "<reason>" --ticket <KEY>` before the first edit, and a work item missed anyway gets a retroactive ticket. [hook:pr-ticket-ref-gate, hook:ticket-at-start-gate]
R-606: Close the ticket (through the R-605 background subagent) only after the verification gate passes, writing `completed_at`, attributable `actual_minutes` (working time, never the calendar gap), `rework_count`, and `estimate_ratio` in one update, then state the R-906 recalibration in one line; abandoned work closes as `dropped` with the reason. [manual]
