...
R-503: For 3+ tasks or any plan/skill execution: announce each task's % share of total work, capture a start timestamp, report cumulative % at each completion and total elapsed time at the end. [hook:session-start, manual]
...
R-514: Never merge a PR without explicit user authorization in the current turn ("merge when ready" is not authorization); direct pushes to `main` only on express request after naming the risks; never request Copilot review. [hook:git-workflow-guard]
