...
R-334: Name every schema, model, type, module, and store base noun first, secondary nouns after, the base noun being the aggregate root that every entity inside the aggregate repeats (`trip_leg`, `conversation_message`); tables and repository modules are plural, foreign keys, enum types, model and schema classes are singular; an aggregate root takes no prefix and a framework's own tables keep the framework's names. [judge]
