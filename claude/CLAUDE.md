...
R-215: Cite a commit in a document only when a ref in that repository reaches it: pin it under a `keep/<slug>` tag and push the tag before the document lands, or mark a citation that is deliberately unreachable with `<!-- unreachable-sha: <sha> <reason> -->` on the citing line. [ci:doc-sha-reachability]
