...
- **File header** (R-320): a `/** what and why */` comment on the first line inside `<script setup>`, never an HTML comment above it, because the header rule reads the script
...
- Every query is wrapped in a composable (`composables/useTripsQuery.ts`) that owns the query key, obtains the API client in setup, and passes it to the `api/` function; components never build query keys inline
...
- **Own-backend calls** go through an openapi-fetch client that `api/apiClient.ts` builds in `createApiClient()` (`createClient<paths>()` over the generated `paths` type) and `useApiClient()` memoizes per Nuxt app instance, never a module-level singleton; each `api/` module stays one function per backend route, takes the client as its first parameter (`fetchTrips(apiClient, query)`), and makes one call on it, which replaces the core's hand-typed `apiFetch` transport. `useApiClient()` runs only in setup, never inside an `api/` function or after an `await`, because `useNuxtApp()` throws once an `await` has dropped the Nuxt context and `experimental.asyncContext` is off by default (added 2026-09-19)
