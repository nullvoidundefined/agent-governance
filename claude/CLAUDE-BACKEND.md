...
Default to parallel, affected-only runs; the full suite belongs to pre-push and CI.
...
- **Pre-push and CI** run the full suite. When one CI job is too slow, shard it across a matrix with `vitest run --shard=<i>/<n> --reporter=blob` and merge with `vitest --merge-reports`. The full suite is the required check before any merge to `main`.
