...
import { createCorsConfig } from "app/config/corsConfig.js";
...
- **Never use `tsup`**
- `tsc-alias` rewrites `app/*` path aliases in compiled output
...
// Builds the session middleware for one environment; the cookie is Secure everywhere but development.
export function createSessionMiddleware(environment: string | undefined) {
    return session({
...
The test drives the real middleware over a request the proxy marks as HTTPS, because express-session withholds a Secure cookie from a plain-HTTP request:

```typescript
import express from "express";
import request from "supertest";
import { describe, expect, it } from "vitest";
import { createSessionMiddleware } from "app/middleware/loadSession.js";

describe("session cookie", () => {
    it("is Secure and HttpOnly in staging", async () => {
        const app = express();
        app.set("trust proxy", 1);
        app.use(createSessionMiddleware("staging"));
        app.post("/login", (req, res) => {
            req.session.userId = "user-1";
            res.sendStatus(204);
        });

        const response = await request(app).post("/login").set("X-Forwarded-Proto", "https");
        const [sessionCookie] = response.get("Set-Cookie") ?? [];

        expect(sessionCookie).toMatch(/;\s*Secure/);
        expect(sessionCookie).toMatch(/;\s*HttpOnly/);
    });
});
```
...
`CORS_ORIGIN` also needs its own shape check, in every environment, not only in production: with `credentials: true` the CORS middleware echoes the configured origin back on every response, so a wildcard or a `null` origin hands the session cookie to any site that asks. `corsConfig.ts` parses the raw value before it ever reaches the CORS middleware:

```typescript
import cors from "cors";

// The values that would open the credentialed API to every site on the web if accepted.
...
const BROWSER_ORIGIN_PATTERN = /^(https:\/\/[a-z0-9][a-z0-9.-]*(?::(?:[1-9][0-9]?|[1-35-9][0-9]{2}|4[0-35-9][0-9]|44[0-24-9]|[1-9][0-9]{3}|6553[0-5]|655[0-2][0-9]|65[0-4][0-9]{2}|6[0-4][0-9]{3}|[1-5][0-9]{4}))?|http:\/\/[a-z0-9][a-z0-9.-]*(?::(?:[1-9]|[1-79][0-9]|8[1-9]|[1-9][0-9]{2}|[1-9][0-9]{3}|6553[0-5]|655[0-2][0-9]|65[0-4][0-9]{2}|6[0-4][0-9]{3}|[1-5][0-9]{4}))?)$/;

// A blank value returns "": no cross-origin caller is allowed.
export function parseCorsOrigin(rawValue: string): string {
    const origin = rawValue.trim();
    if (origin === "") {
        return "";
    }
    if (UNSAFE_CORS_ORIGINS.has(origin.toLowerCase())) {
        throw new Error("CORS_ORIGIN must name one concrete origin, not a wildcard or null");
    }
    if (!BROWSER_ORIGIN_PATTERN.test(origin)) {
        throw new Error("CORS_ORIGIN must be scheme://host[:port] exactly as a browser sends it");
    }
    return origin;
}

// Builds the CORS middleware from the raw CORS_ORIGIN value when the app starts, never at import time.
export function createCorsConfig(rawOrigin: string | undefined) {
    const origin = parseCorsOrigin(rawOrigin ?? "");
    return cors({ origin: origin === "" ? false : origin, credentials: true });
}
```

```typescript
import { describe, expect, it } from "vitest";
import { parseCorsOrigin } from "app/config/corsConfig.js";

describe("parseCorsOrigin", () => {
    it("returns a real origin unchanged", () => {
    ...
    it.each([
        ...
        ["https://a.example,https://b.example", "comma-separated list"],
        ["https://client.example/path", "origin with a path"],
        ["https://name@client.example", "origin with userinfo"],
    ])("refuses %s (%s)", (unsafeOrigin) => {
        expect(() => parseCorsOrigin(unsafeOrigin)).toThrow(/CORS_ORIGIN/);
    });

    it.each(["", "   "])("returns an empty string for the blank value %j", (blankValue) => {
        expect(parseCorsOrigin(blankValue)).toBe("");
    });
});
```
...
app.set("trust proxy", 1);
app.use(helmet());
app.use(createCorsConfig(process.env.CORS_ORIGIN));
app.use(requestLogger);
app.use(rateLimiter);
app.use(express.json({ limit: "10kb" }));
app.use(cookieParser());
app.use(csrfGuard);
app.use(createSessionMiddleware(process.env.NODE_ENV));
...
## Risky Migrations and Large-Scale Schema Changes

Any migration that renames tables, drops tables, moves data between tables, renames FK columns, or changes CHECK constraints on columns with existing data is a risky migration. For these:

1. **Develop locally.** Write and iterate the migration against local Postgres until it runs cleanly.
2. **Validate on a Neon branch.** Create a branch off the staging database. Run the full migration against the branch. Verify:
   - Row counts match expectations (no data lost or duplicated)
   - All FK dependents resolve (no orphaned rows with dangling foreign keys)
   - CHECK constraints pass for all migrated rows (especially enum-like columns with new or changed value sets)
   - The full test suite passes against the branched database
3. **Deploy to staging.** Run against real staging only after the branch validation passes. Smoke test all affected flows.
4. **Deploy to production.** Only after staging is green for a full cycle.

If the migration fails at step 2, delete the branch and iterate.

Migrations that only add new tables, add nullable columns, or add indexes are not risky and do not require this process.
...
Default to parallel, affected-only runs; the full suite belongs to CI.

- **Parallel by default.** Vitest runs test files in parallel workers; never turn that off globally (`--no-file-parallelism`, `fileParallelism: false`, a single-thread pool) to hide shared state. Give each worker its own state instead: a per-worker database or schema keyed on `VITEST_POOL_ID`, unique temp directories, no shared ports.
- **Turn ends, commits, and branch-level merges** (merging `main` into a feature branch) run only the affected tests: `vitest run --changed origin/main` (or `--changed` alone for uncommitted work), or `vitest related --run <changed files>`. A change to shared test setup (`vitest.config.*`, global setup files, test utilities every suite imports) runs the full suite.
- **CI** runs the full suite; pre-push does not (IAN-98). When one CI job is too slow, shard it across a matrix with `vitest run --shard=<i>/<n> --reporter=blob` and merge with `vitest --merge-reports`. The full suite is the required check before any merge to `main`.
