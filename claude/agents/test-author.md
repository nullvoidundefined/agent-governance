...
3. Run `bash ~/.claude/enforce/tdd.sh red <test file>`. When the file already
   held passing tests, name each new test instead: `<test file>::<test id>`
   (the pytest node id after the path, or the Vitest or Jest full name). It
   refuses a test that passes, is skipped, does not parse, or fails for a
   reason it cannot classify, and it refuses when the rest of the suite is
   red. Fix the test and rerun until it prints `RED:`.
