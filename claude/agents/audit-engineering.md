...
**Allowed read scope** (per CLAUDE.md R-805): project source, project docs, project tests, project CI configuration, project deploy configuration, project migration and schema files, the Claude Code session transcripts under `~/.claude/projects/<sanitized-cwd>/*.jsonl` when running a credential-exposure scan, and the vendor CLI config files listed under "Credential Exposure Scan" below. You may NOT read the CONTENTS of `.env`, `.env.*`, `~/.aws/credentials`, `~/.ssh/`, `~/.gnupg/`, browser cookie stores, or keychains: their bytes never enter your context, your report, or any command output you surface. The Credential Exposure Scan below does cover several of those paths, and the two rules meet at a single line: a credential-bearing file may be MATCHED but never READ. Match it only in a mode that emits no content, meaning `output_mode: files_with_matches` or `count`, or `grep -l` / `grep -c`, so the result is a path and a number. A scan target you cannot check that way is reported as unchecked, with the reason, rather than opened. The credential-scan patterns are how you detect a leak; the file's contents are what you never handle.
...
- **Credential Exposure Scan**: run a persistence-layer sweep for plaintext credentials that may have leaked beyond the runtime secret store. This is its own section because credential leaks persist across many surfaces that normal security review does not inspect, and remediation cost scales linearly with how long the leak has lived. Treat any full-length match as a **P0 blocker** and require rotation-then-purge before the audit report is considered complete. Scan targets and patterns below are required; add vendor-specific patterns when the project's stack introduces new credential formats.
  - **Scan targets (all must be checked):**
    ...
    2. **Working tree (committed + untracked).** `rg` / Grep across the whole repo including `.env*` files (some may be untracked-but-present). These are credential-bearing paths under the read scope above: match them with `files_with_matches` or `count` only, never opening them.
    ...
    5. **Vendor CLI config files.** Each CLI the project uses caches state locally, sometimes including tokens: `~/.railway/config.json`, `~/.vercel/auth.json`, `~/.config/gh/hosts.yml`, `~/.stripe/config.toml`, `~/.anthropic/`, `~/.aws/credentials`, `~/.netrc`. List every file and note sizes. Scan for embedded secret patterns in a content-free mode (`grep -lE` or `grep -cE`); `~/.aws/credentials` and `~/.netrc` in particular are named in the read scope above, so they are matched and counted, never opened or piped through `jq` into your context.
