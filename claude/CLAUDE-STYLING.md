...
Vue composes the same classes with the `:class` array form; a falsy entry is dropped, so no empty-string branch is needed:

```vue
<button :class="[styles.chip, isSelected && styles.chipSelected]">
```
...
### SCSS Module Import in Vue

The sibling `.module.scss` file is imported inside `<script setup>`, the same as in TSX, so the per-component-folder rule holds for both frameworks; an SFC never carries a `<style module>` block:
...
<template>
    <div :class="styles.chatBox">
        <div :class="[styles.message, styles.user]"></div>
    </div>
</template>
```

- Import as `styles` (always this name), exactly as in TSX
- Compose multiple or conditional classes with the `:class` array form, never a template literal
