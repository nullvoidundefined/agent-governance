...
These rules apply to all web-client and frontend packages across every app in this portfolio. This file holds everything framework-agnostic; component syntax, state primitives, and framework-specific ESLint rules live in the framework files below, and structure, routing, entry files, and env vars live in the framework's SSR or bundler file.

---

## Framework Files

Detect the framework from the project root and read the matching files alongside this one:

| Marker | Framework | Read |
|--------|-----------|------|
| `next` in `package.json` dependencies | Next.js (App Router) | `~/.claude/CLAUDE-FRONTEND-REACT.md` and `~/.claude/CLAUDE-FRONTEND-NEXT.md` |
| `vite.config.ts` in project root with `react` in `package.json` | Vite (React SPA + TanStack Router) | `~/.claude/CLAUDE-FRONTEND-REACT.md` and `~/.claude/CLAUDE-FRONTEND-VITE.md` |
| `nuxt.config.ts` in project root | Nuxt 4 (Vue 3 SSR) | `~/.claude/CLAUDE-FRONTEND-VUE.md` and `~/.claude/CLAUDE-FRONTEND-NUXT.md` |
...
- **A server-state cache** (TanStack Query for the framework) for all server state; no hand-rolled fetch-in-effect
...
Shared across frameworks (R-305). The routing directory, entry files, and the state directory's name are framework-specific; see the framework file.

```
src/                          # `app/` under Nuxt; the framework file names the root
├── components/               # Shared UI components, one folder per component
│   └── Header/
│       ├── Header.<ext>      # .tsx under React, .vue under Vue
│       └── Header.module.scss
...
├── state/                    # Stores and providers (React); `composables/` and `stores/` under Vue
...
- Each component gets its own folder: `ComponentName/ComponentName.<ext>` + `ComponentName.module.scss`
- Never `lib/`, `utils/`, `helpers/`, or other catch-alls (R-306); classify into `api/`, `clients/`, `services/`, or the framework's state directory
...
Framework-specific rows (components, hooks or composables, pages, layouts, routes, global styles) live in the framework files.
...
Imports are grouped with blank lines between groups, in this order; the framework file shows the concrete example and names the packages in group 1:

1. Framework imports (the framework's runtime and its own packages)
...
- Use `@/` path alias for the source root; never relative `../../` beyond one level

---

## TypeScript Patterns

- **Interfaces** for component props; **types** for unions and utility types: `type Status = 'idle' | 'loading' | 'error'`
- **Zod-inferred types** when shared with backend: `type Job = z.infer<typeof jobSchema>`
- Props types live in the same file as the component, above it (the framework file shows the declaration form)
...
- Components consume these functions through the server-state cache's query and mutation hooks, never by calling them inside an effect or lifecycle callback
- The cache client config lives in `config/queryClient.ts`
...
- The server-state cache's `onError` callbacks route to toast
...
Template-attribute quoting is a framework row (the React file sets `jsxSingleQuote`; the Vue file sets the template attribute style).
...
- No explicit `any`
- Framework plugin rules are listed in the framework file
...
- CI runs the full suite; pre-push does not (IAN-98). Shard it across a CI matrix with `--shard=<i>/<n>` when one job is too slow. The full suite is the required check before any merge to `main`.
...
- **Assert data and behavior, never element presence alone.** A test that only checks a component rendered does not prove the flow works; assert the value the user sees or the state the server holds.
