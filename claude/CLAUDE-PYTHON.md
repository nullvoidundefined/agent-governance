...
- **redis.asyncio** for rate limits, the circuit breaker, and the job queue
...
def register_middleware(app: FastAPI, settings: Settings) -> None:
    ...
    app.add_middleware(RequestContextMiddleware)                 # 1
```

The request passes them in this order:

1. **Request context**: honor or mint the request ID, bind it to structlog, echo it on the response, and reject a body over 100 KB with 413 unless the route is on the upload allowlist
...
- TLS is required in production through the URL's `ssl=require` parameter
...
- The ledger table `billing_webhook_events` (R-334; `stripe_event_id` unique, `event_type`, `status` in `claimed`, `processed`, `failed`, `attempted_at`) makes delivery idempotent: insert with `ON CONFLICT DO NOTHING` to claim, skip if already `processed`, mark `processed` or `failed` in the same transaction as the side effect
...
- `services/circuit_breaker.py` guards calls to a flaky provider: after 5 failures inside 60 seconds the circuit opens for 30 seconds, and calls fail fast with a typed `CircuitOpenError` the caller handles
- State (failure count, opened-at time) lives in Redis under `circuit:{provider}` so every instance shares it
- Without Redis the breaker fails open: it never blocks a call, and it logs one `circuit_breaker_disabled` warning at startup
- The breaker wraps the client call inside `with_client_telemetry`, so an open circuit still logs its outcome
...
def configure_logging(settings: Settings) -> None:
    """Install the processor chain and renderer for this environment."""
    renderer = (
...
- Errors pass as `exc_info=err`, which `dict_tracebacks` renders as a structured field, the Python form of `{ err }`
- uvicorn's access and error loggers are routed through the same renderer, so every line in production is JSON. No secrets, tokens, passwords, emails, or other PII in any field (R-104); log IDs instead

---

## Observability (R-341 to R-346)

**Request ID (R-341).** One ID per request, honored from the caller, echoed back, and bound to the context so nothing passes it by hand:

```python
"""Binds a request ID to every log line and echoes it on the response."""
...
class RequestContextMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        """Honor an inbound X-Request-Id or mint one, bind it, and echo it."""
        if scope["type"] != "http":
            await self.app(scope, receive, send)
            return
        inbound_id = dict(scope["headers"]).get(REQUEST_ID_HEADER, b"").decode()
        ...
        async def send_with_request_id(message: Message) -> None:
            if message["type"] == "http.response.start":
                message["headers"] = [*message.get("headers", []), (REQUEST_ID_HEADER, request_id.encode())]
            await send(message)
...
- Health endpoints (R-345) are the two under Health Endpoints; workers answer through `arq --check`

---

## pg_cron Cleanup Jobs

- A migration runs `CREATE EXTENSION IF NOT EXISTS pg_cron` inside a `DO` block that catches `insufficient_privilege` and `undefined_file`, so it succeeds where the extension is unavailable
- Where pg_cron exists, the same migration schedules hourly deletes: expired `user_sessions`, `request_idempotency_keys` older than 24 hours, and `billing_webhook_events` older than 30 days
- Where it does not, an arq cron job (`cron(delete_expired_rows, minute=0)`) runs the same deletes; the job checks `cron.job` first and exits when pg_cron already owns the work
- Deletes run in batches of 1000 so a large backlog never holds a long lock
...
- The same gate gains `D100` and `D103` (module and public-function docstrings, the R-320 analog) in slice 01 PR 5
