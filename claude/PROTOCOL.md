...
Convention files (`~/.claude/CLAUDE-BACKEND.md`, `CLAUDE-FRONTEND.md`, `CLAUDE-FRONTEND-REACT.md`, `CLAUDE-FRONTEND-NEXT.md`, `CLAUDE-FRONTEND-VITE.md`, `CLAUDE-FRONTEND-VUE.md`, `CLAUDE-FRONTEND-NUXT.md`, `CLAUDE-DATABASE.md`, `CLAUDE-STYLING.md`, `CLOUD-DEPLOYMENT.md`, `KNOWN-ISSUES.md`) are read on demand when the task touches that layer, never preloaded. This keeps context lean and forces Claude to consult conventions deliberately rather than passively.
...
- **Layer 5 gained the slice loop.** The TDD harness assessment (`docs/audits/2026-09-06-tdd-harness.md`) asked one question of every mechanism: if the implementing model is wrong, or is optimizing for green, what stops it. The answer for test mutation, gate inputs, RED evidence, and subagent turns was "prose". Three rules and two artifacts close that: `hooks/protected-path-guard.sh` (R-410 gate inputs and locked paths, R-411 role boundaries by `agent_type` against `enforce/role-policy.json`, R-412 slice phases), `enforce/tdd.sh` (open, red, green, close: a RED is accepted only for an assertion or missing-module failure with the rest of the suite green, and a GREEN only when the locked files match the lock and the RED commit), and `verification-gate.sh` on `SubagentStop`, skipping only roles that write nothing. Superpowers stays the process skeleton; the harness enforces the boundaries its skills describe. Decisions recorded in the assessment: Vitest first, Standard tier enforced in one session, Opus for the test author and critic in Complex and Saga, disputes to the human, gate inputs denied outright, the judge key still pending.

## What changed on 2026-07-31 and 2026-08-01

- **Layer 3 gained three stack tracks.** Python (2026-07-31 morning), then Ruby on Rails and Go (same day): `CLAUDE-PYTHON.md`, `CLAUDE-RUBY.md`, `CLAUDE-GO.md`, each carrying the stack's analogs of the `[ts]`-tagged rules and its blessed exceptions (Python `app/`+`db/`+`core/`, Ruby snake_case+`lib/`+predicate-`?`, Go co-located `*_test.go` as a toolchain requirement, waived dir-case, idiomatic short names). Layer 6 gained matching AST-tier push gates: ruff, RuboCop, golangci-lint, joining ESLint; the manifest, the R-516 closure guard, and the fixture suites learned the new enforcer prefixes. The criticism audit's caveat stands in ISSUES.md: these tiers are speculative until real projects in those stacks exist.
- **Three full-harness audits ran (engineering, criticism, security) and their remediation landed the same day.** Highlights: push gates no longer execute code from the target repo (`bundle exec` banned, Go gate opt-in via `gate-trusted-repos.txt`); plaintext API keys found in local transcripts were purged and the emission class documented (`redact-output.sh` now states its honest post-hoc semantics: PostToolUse cannot rewrite a tool result); the handoff loop was discovered dead (`session-start.sh` read `docs/audits/` while R-602 writes `docs/session-handoff/`) and fixed with SHA verification; chained `git add && git commit` no longer bypasses the commit guards; a hash-manifest integrity guard now covers hook content; the publish guard fails closed; the inert llm-judge tier (no key in the hook environment since inception, concealed by stub-only tests) now warns at session start until provisioned or explicitly accepted.
- **Layer "measurement" became mechanical.** Every deny/ask hook appends to `telemetry/rule-fires.log` via `log-rule-fire.sh`; `session-end.sh` rolls new fires into `global-memory/rule_fires.md` as counted entries. The criticism audit's core finding was that the fire log had recorded nothing while hooks quadrupled; the loop now feeds itself.
- **Layer 6 permission posture: strict interpreter lockdown applied (security audit P0) and reverted the next day by the maintainer.** One day of `bash`/`node`/`python3` prompting proved the friction unacceptable for solo daily work; the lenient allow list returned, destructive deny/ask rules stayed, the strict variant is preserved in `enforce/strict-permissions.json`, and the residual bypass is recorded in ISSUES.md as an accepted risk. A deliberate, documented tradeoff beats a silently reverted one.

## What changed on 2026-07-29

- **Layer 3 restructured for context adherence.** `CLAUDE.md` shrank from ~460 lines to one norm line per rule (under 200 lines total), after an industry audit found the always-loaded prompt 3-5x past published instruction-adherence thresholds (compliance decays roughly linearly with instruction count). Full Spec text moved verbatim to `rulebook/reference.md`, read on demand. The enforcement guard (`enforcement-guard-check.sh`) and the push-time judge (`llm-rule-judge.sh`) now read rule text from `rulebook/reference.md`; the judge gained full Spec blocks where it previously saw only norm lines. The principle applied: a rule with a deterministic enforcer no longer needs its prose in the prompt (R-516 in reverse).
- **Layer 6 gained a harness-level permission layer.** The blanket `Bash(*)` allow (single-layer posture: hooks were the only gate on shell commands, and the push gates self-describe as evadable) was replaced with a usage-mined allowlist plus ask rules (force-push, `--no-verify`, `gh pr merge` per R-514, `sudo`, `rm -rf`) and deny rules for catastrophic classes. Two standing guards joined the enforce suite: `hook-latency.test.sh` (per-edit hook chains stay cheap; measured baseline ~6ms per event, so dispatcher consolidation was rejected as unjustified) and `claude-md-lint.test.sh` (CLAUDE.md stays under 200 lines, norm lines and reference Specs stay in sync, nothing un-frontmattered creeps into `rules/`).
- **Tier 2 loading became mechanical.** Claude Code auto-loads every un-frontmattered `~/.claude/rules/*.md` at launch, which had silently defeated the read-on-demand design: agents.md, audits.md, and cost.md were loading into every session. Those files moved to `~/.claude/rulebook/` (outside the auto-load zone), restoring R-001's tiering; `session-types.md` stays in `rules/` deliberately, since classification is needed every session. The stack convention files (`CLAUDE-BACKEND.md` and siblings) gained `paths:` frontmatter and are symlinked into `rules/`, so they now load mechanically when work touches matching files instead of by recall, migrating another prose layer down to a mechanical one.
...
- **R-341 to R-346 (new 2026-09-04):** `CLAUDE-BACKEND.md` had described Pino logging, `pino-http` request IDs, two health endpoints, and a global error handler for months without any of it being a rule; a `console.log` in a handler, a typo in an analytics event literal, and a `catch {}` around a provider call all passed every gate. The maintainer asked for enforced observability rules; the decidable parts became three ESLint rules plus `no-console` and `no-empty` scoped to the server trees, and the rest carry Spec blocks and `[manual]`.
- **R-351 (new 2026-09-05):** `CLOUD-DEPLOYMENT.md` had carried a per-service Dockerfile strategy for months while nothing required a Dockerfile to exist; a new service could reach its first deploy on a platform buildpack, and a container arrived later as a retrofit, if at all. The maintainer asked that every new project be Dockerized from the start, one container per deployable artifact; the rule names the artifact set, the image contract (multi-stage, pinned base, non-root, healthcheck, `.dockerignore`, compose), and a write-time reminder that fires when an artifact marker lands with no Dockerfile on the path to the repo root.
- **R-401 (old R-200):** Each listed anti-pattern was observed in real committed tests before being banned.
- **R-410, R-411, R-412 (new 2026-09-06):** The 2026-09-06 TDD harness assessment (`docs/audits/2026-09-06-tdd-harness.md`) found that every defense against an implementer weakening a test was prose: `tdd-gated-dispatch` said "do not modify the pre-written test files" and nothing checked, the Stop-time verification gate ran whatever `.claude/verify.sh` said, and RED was a checklist line. The three rules make the slice loop mechanical: a lock written by `enforce/tdd.sh` that phases from open to red to green, a guard that denies writes by path and by subagent role, and a hash comparison at GREEN against the RED commit.
