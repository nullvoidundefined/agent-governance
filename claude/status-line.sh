...
# its input is null or absent, and every failure path still prints a line
...
if [ -n "$CTX" ] && CTX_FMT=$(printf '%.0f' "$CTX" 2>/dev/null) && [[ "$CTX_FMT" =~ ^-?[0-9]+$ ]]; then
  CTX="${CTX_FMT}%"
else
  CTX="-"
fi
...
if [ -n "$RATE" ] && RATE_FMT=$(printf '%.0f' "$RATE" 2>/dev/null) && [[ "$RATE_FMT" =~ ^-?[0-9]+$ ]]; then
  RATE="$RATE_FMT"
else
  RATE="-"
fi
