...
| `llm-judge` | `judge-diff.sh` (a fast model over the diff), run by the `rule-judge` CI workflow | per pull request | R-315, R-316, R-317, R-325, R-334, R-362, R-363, R-364, R-365 |
...
The row above lists exactly the manifest's `llm-judge` rows and nothing else. R-362 to R-365 joined it on 2026-09-25 at `warn` severity: the data-access rules' intent halves (whether writes needed a transaction, unguarded read-modify-write, unbounded reads, SQL built from caller input) are not decidable from the AST, and a warn row annotates the pull request without blocking it on a probabilistic verdict. It listed seven rules until 2026-09-17 (audit P2-4): R-320 and R-322 left the tier in the 2026-09-04 reclassification as pure AST questions, R-318's own Spec in `rulebook/reference.md` argues against ever judging it ("a non-deterministic verdict on an undecidable property is confidence theater"), and R-325's documented judge half had no manifest row, so the judge never evaluated the one thing the doc said it decided. Only that last one was a defect, and it is fixed; the other three were the table drifting ahead of decisions already taken.
...
`hooks/protected-path-guard.sh` runs on every Write, Edit, and Bash call and decides from two inputs. The first is `.claude/tdd-lock.json` at the repo root, written by `enforce/tdd.sh`: phase `open` (a slice is declared, its failing test not yet proven) denies production writes; phases `red` and `green` deny every test-tree write plus the fixture prefixes and spec path the lock lists, so the tests are the contract from `tdd.sh red` to `tdd.sh close`. The second is the `agent_type` field the hook input carries in subagent context, matched against `role-policy.json`: `test-author` writes only test and fixture trees, `implementer` never writes tests, fixtures, specs, or the lock, `slice-critic` writes nothing; an absent or unlisted type carries no role restriction. Independently of both, the gate inputs (`.claude/verify.sh`, `.enforce.json`, `.enforce-baseline.json`, the lock) are never written by a session, and test-runner configs plus the `package.json` test and typecheck scripts ask. Bash is judged by what it writes, never by the text it carries: quoted strings and heredoc bodies are set aside before the command is scanned, because on 2026-09-24 a Vitest `-t` filter containing the word "rm" and a `=> {` inside a `python3 - <<EOF` body each stopped a build. The write targets are redirections, `tee`, the path operands of `rm`, `mv`, `shred`, `truncate`, `unlink`, `sed -i`, `perl -i`, `git rm|mv|checkout|restore|clean|stash`, and `find -delete|-exec`, the destination alone of `cp`, `rsync`, `install`, and `ln` (so copying a locked test elsewhere is a read), a nested shell's own targets (`bash -c`, `eval`, `bash <<EOF`), and the path literals an inline interpreter script (`python -c`, `node -e`, `python3 - <<EOF`) hands to a write call, on the same line or through a variable assigned from a literal. A script file run by name, or a write through a path built at run time, is not seen here; `tdd.sh green` compares locked-file hashes against the RED commit for that case. Phase `amending`, opened by `tdd.sh amend`, leaves exactly one path writable, the test under amendment. Paths outside the repository root are not governed. Fixtures: `tests/protected-path-guard.test.sh` and `tests/protected-path-guard-bash-parsing.test.sh`.

`tdd.sh` is the evidence half. `tdd.sh open "<slice>" [--spec <path>]` writes the lock in phase `open`. `tdd.sh red <test file>...` runs the whole suite once (Vitest or Jest from the project's `node_modules/.bin`, else the copy bundled here; or, when every named file is a `*.test.sh` bash fixture, every `*.test.sh` in those files' directories, run through `run-fixture-shards.sh --results-dir` from a scratch working directory (so a fixture's relative writes never land in the slice's tree) with the fixture suite's own verdict and converted to the same JSON report, so a failed fixture's FAIL line is the assertion RED and bash's "No such file or directory" or "command not found" for a script that does not exist yet is the missing-module RED; or, when every named file is a `*.py` pytest test, the whole pytest suite of the nearest directory above them holding `pyproject.toml`, run there under `uv run python` (the environment `uv run pytest` uses; or the project's `.venv` interpreter, `python3`, or `python`, with a warning, when uv is not on PATH) through a short bootstrap that sets `sys.pycache_prefix` to a fresh per-run directory and disables bytecode writes from inside the interpreter (environment variables are not enough, since a pytest console script with `-E` in its shebang ignores them), so a stale `.pyc` in the tree can never fake a GREEN, with `--continue-on-collection-errors`, its built-in `--junitxml` report in the xunit1 family converted to the same JSON report, so an `AssertionError` or an unmet `pytest.raises` is the assertion RED, an `ImportError` or `ModuleNotFoundError` is the missing-module RED, and a `SyntaxError` at collection is a test that does not parse; a slice never mixes runners) and accepts only when every test in the named files fails for an assertion or a missing-module reason and no other file fails; a syntax error, a file with no tests, a passing test, or a skipped test is refused with the reason; under Vitest and Jest the assertion RED is the runner's own assertion output, read with ANSI colour stripped (an `AssertionError`; a Jest matcher hint such as `expect(received).toBe(expected)`, `expect(jest.fn()).lastCalledWith(...expected)`, `expect.assertions(1)`, or jest-circus's `assert.strictEqual(received, expected)`; a Vitest `.resolves`, `.rejects`, `expect.poll`, or `expect.extend` failure, known by its wrapper's stack frame; a snapshot mismatch; and Vitest's `expect.assertions` and `expect.hasAssertions` failures; a Jest `expect.extend` matcher counts only when its message carries `this.utils.matcherHint`, since without one its failure cannot be told from a plain error), so a plain `Error` whose message merely says "expected" or names a matcher is refused (one that quotes a marker verbatim, such as `call expect(x).toBe(y) first`, is still accepted, since the reports carry no error type to tell it apart); and each test is classified on its own, so one failing for any other reason (a plain error, a timeout) is refused by `<test file>::<test id>` even when another test in the file fails with an assertion. An argument may instead name tests inside a file, `<test file>::<test id>`, for a slice that adds tests to a file already holding passing ones: under pytest the id is the node id after the path (`test_x`, `TestA::test_x`, `test_x[1-2]`, where a bare parametrized name covers every parameter set and a class every test in it, read from the JUnit classname and name), and under Vitest and Jest it is the test's full name (describe titles and the title joined by spaces, the string their `-t` filter matches). The named tests must fail for the same reasons, every other test in that file must neither fail nor be skipped, a suite-level error in that file (a throwing `afterAll`) is refused, a file named both whole and by id is refused, and the file must still load, so an id cannot match nothing and a new test imports an unwritten unit inside its body; bash fixtures stay file-level and a test id on a `*.test.sh` path is refused, because each fixture is one test. It records the pass count outside the named tests as the baseline (the unnamed tests of an id-named file count toward it), the sha256 of each containing file with the named ids beside it, and moves to `red`. `tdd.sh green` first compares the containing files against the lock and, when the lock is committed, against the commit that introduced it (the RED commit), then runs the suite and requires every named test to pass, none skipped, no other failure, and the outside count at or above the baseline; it moves to `green` and is re-run after every refactor. `tdd.sh close` removes the lock from `green`, or from `open` when no test was ever locked, because nothing can have been written under a lock that never reached `red`; before this, a slice whose first `red` was refused could only be ended by the user deleting the lock. One failure outside the named files does not refuse the red, and only the red: `tests/hook-hashes-closure.test.sh` reporting integrity-manifest drift whose every reverse-closure line names one of the test files the red command named, in the harness-relative or the repository-root spelling and in no shorter trailing run of components, because writing a slice's own fixture is what puts an unhashed file under `enforce/tests/` and without this exemption no test author could ever reach a RED in this repository (IAN-156); drift naming any other path, a run with no reverse-closure line to bound, and any other failing fixture all still refuse, as does `tdd.sh green`, where the drift would come from the production file the implementer edited rather than from the slice's own fixture (R-517 review of PR #91). The bound is the reverse-closure axis alone: the fixture collapses content drift to a count before reporting it, so that half of its signal is muted rather than bounded, and CI running the closure fixture is what still catches it (IAN-184). `tdd.sh expected-red` is the same question asked from outside, for `hooks/verification-gate.sh`, which refuses to let a turn or a subagent end on a red suite (R-509) and so blocks a test author on the one outcome its role exists to produce, and which IAN-184 wires to this subcommand: it exits 0 when a lock is in phase `red` and every failure in the suite lies in the files the lock records, non-zero for no lock and for phases `open`, `green`, and `refactor`, and it writes nothing at all, because the caller runs it on a tree it is about to block or release. It asks about membership and not identity: it does not re-check that the locked files still fail, nor compare their hashes against the RED commit the way `green` does. Fixtures: `tests/tdd-red-manifest-drift.test.sh` and `tests/tdd-expected-red.test.sh`. `tdd.sh validate <role>` is the orchestrator's check on a dispatched role's return (2026-09-17 skills audit): the phase that role leaves behind (red after `test-author`, green after `implementer`), every modified or untracked path inside the role's `role-policy.json` boundary with the lock itself excepted, and for the implementer a re-run of `green`; the tdd-gated-dispatch skill runs it once per role instead of reading porcelain output against the patterns by hand. `tdd.sh open --refactor` is the no-RED variant for behavior-preserving work: the suite must be green at open, the named test files (or every test file the suite ran) are hashed and locked, the phase is `refactor` (the guard treats it like `red`), and `tdd.sh green` proves the same tests pass unchanged. `tdd.sh amend <test file>` is the author's way out of a bug in the test it just proved RED, which before this needed the user to delete the lock: from phase `red` it opens phase `amending`, in which the guard lets only that file be written; run again, it re-runs the suite, requires the amended test to still fail for a classified reason with nothing outside the locked tests failing, re-hashes it, appends `{path, fromSha256, toSha256, failureClass, at}` to the lock's `amendments`, and returns to `red`. It is refused for a test the slice did not lock (an earlier slice's test stays under R-410), in any other phase (after green above all), and once a remote-tracking ref reaches the RED version of the test or a committed lock recording it; a lock some earlier slice committed and pushed does not count. `tdd.sh abandon` closes a lock left by a dead session without the user and appends one JSON line to `CLAUDE_TDD_ABANDON_LOG` (default `telemetry/tdd-abandon.jsonl` under the Claude home); it is refused for a lock that never recorded a test (`close` handles that), for a lock whose newest timestamp or mtime is younger than `CLAUDE_TDD_STALE_HOURS` (default 4), while `lsof` shows a live process outside the caller's own session (its topmost ancestor working in the tree and everything below it) working in the tree, while a locked test is untracked or differs from HEAD or the lock's hash, and while a locked test or anything else in the suite fails. Fixtures: `tests/tdd-amend.test.sh`, `tests/tdd-amend-old-lock-commit.test.sh`, `tests/tdd-abandon.test.sh`. go test and RSpec are refused until a project on that stack exists. Fixtures: `tests/tdd-red-green.test.sh`, which drives the Vitest pinned in `package.json` against one throwaway project and bash fixtures against a second, and `tests/tdd-pytest.test.sh`, which drives the real pytest behind a stub `uv` and a stub `.venv` interpreter against a throwaway project below its repository root.
