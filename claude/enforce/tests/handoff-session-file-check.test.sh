...
# The index is the six sections plus the dated list of session files.
index() { sections; printf '\n## Sessions\n\n- 2026-09-20 ian260 split: `docs/session-handoff/2026-09-20-ian260-split.md`\n'; }
...
# --- The index owes a Sessions list; a session file must not be asked for
# one. Without the list the index stops being a route to the session files,
# which is the only thing making the split navigable. ---
sections > "$SB/index-nosessions.md"
OUT=$(run "$INDEX" "$SB/index-nosessions.md")
check "index without a Sessions list is named" reports "no session list"

index > "$SB/index-good.md"
OUT=$(run "$INDEX" "$SB/index-good.md")
check "compliant index is silent" silent

OUT=$(run "$SESSION_FILE" "$SB/session-good.md")
check "session file is not asked for a Sessions list" silent
