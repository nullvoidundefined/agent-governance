...
printf 'exit 0\n' > "$REPO/claude/hooks/tests/run-tests.sh"
GOT=$(gate "$REPO")
printf '%s' "$GOT" | grep -qF 'ENFORCE_ARGS[--affected]' || { echo "FAIL: the gate must run the enforce suite with --affected, got: $GOT"; exit 1; }
printf 'exit 0\n' > "$REPO/claude/enforce/tests/run-tests.sh"
printf 'echo "HOOKS_ARGS[$*]"\nexit 1\n' > "$REPO/claude/hooks/tests/run-tests.sh"
GOT=$(gate "$REPO")
printf '%s' "$GOT" | grep -qF 'HOOKS_ARGS[--affected]' || { echo "FAIL: the gate must run the hook suite with --affected, got: $GOT"; exit 1; }
