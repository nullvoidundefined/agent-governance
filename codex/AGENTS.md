<!-- GENERATED by openai/build.mjs from CLAUDE.md and rules/session-types.md. Do not edit; change the source and run: node openai/build.mjs --write -->
...
The canonical rule file, loaded into every session. One norm line per rule; the complete Spec, Scope, and Enforcement detail for every rule lives in `~/.claude/rulebook/reference.md`, read on demand: before structural or naming decisions (R-3xx), before test design (R-4xx), or whenever a norm line is not enough to act on. The conditional, mechanically enforced structure rules live in the `/structure-conventions` skill rather than here. The trailing bracket names the enforcer: `[manual]` depends on recall; `[judge]` is the push-time LLM judge; hooks and ESLint fire mechanically. Under Codex the hooks fire through `~/.codex/hooks.json` (generated from `settings.json`; `~/.codex/hooks/codex-hook-adapter.sh` replays each `apply_patch` as the file edits the gates read and translates the ask decision); a tag reading `hook:X in Claude Code; manual in Codex` names a hook with no Codex event, so that rule depends on recall here. `[ts]`/`[py]` after a rule ID scope it to a stack. Rationale and history: `PROTOCOL.md`.

Project-level `CLAUDE.md` adds guidance but does not override these unless it explicitly says so.

## Session init (R-0xx)

R-001: Run the session-start procedure before any other work: (1) confirm the `SessionStart` hook injected `~/.claude/global-memory/INDEX.md` and the SHA-verified `docs/session-handoff/session-handoff.md`; Read either only if its block is absent, verifying the handoff's recorded SHA with `git cat-file -e`; (2) classify the session type per the Session types section below; (3) read that type's Tier 2 files; (4) `git status -s ~/.claude`, triage non-empty; (5) read the project `AGENTS.md` or `CLAUDE.md`; Claude Code's per-project auto memory does not load under Codex. First line of the response after the reads: `Session: <type> | Loaded: <files or "core only"> | Skipped: <files>`. Re-read and re-declare on reclassification. [manual]
...
R-210: Explain every technical term on first use in a message to the user: what it is, where it runs, and why it matters to them; never name an infrastructure component (a proxy, sandbox, network policy, container, hook event) as a bare phrase. [manual]
...
R-502: Track user-visible workstreams as tasks (Codex's plan; `TaskCreate` in Claude Code), not inline sub-steps. [manual]
...
R-504: Commit after every discrete task; marking a task completed triggers an immediate commit. [hook:task-commit-reminder in Claude Code; manual in Codex]
...
Codex has no path-scoped loading: before editing a surface, Read the `CLAUDE-*.md` convention file the Session types section maps to it, and read it when planning that layer before any file is open.
...
Session type is orthogonal to stack. Nothing auto-loads by path under Codex. Detect the stack from the root marker files and Read the matching convention file(s) before editing that surface:
...
The global memory index `~/.claude/global-memory/INDEX.md` is injected by the `SessionStart` hook (R-001, R-002). If its block is absent from context, Read it before starting work; Read the individual memory files it lists only when their one-line summary is the reason you need them.
