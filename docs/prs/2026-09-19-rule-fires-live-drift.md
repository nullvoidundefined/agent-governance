...
- **Existing history.** The committed log contents stay in git history and in every live `~/.claude` that already has them, because sync.sh never deletes and git only removes the file from checkouts that pull this commit.
