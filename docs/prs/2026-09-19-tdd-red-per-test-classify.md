...
- `claude/enforce/tdd.sh`: the per-test loop that PR #74 wrote inside `classify_named` moves into a new function, `classify_failures <rel> <results>`. It reads one `{key, failures}` object per failing test, one per line of `<results>`, refuses the first test whose messages match neither class with `<rel>::<test id> fails for a reason this script does not classify: <first line>`, and prints `missing-module` when any test is missing-module and `assertion` otherwise. `classify_red` and `classify_named` both call it directly, so a refusal's `die` exits the `$(...)` that `cmd_red` captures the class with. The only difference between the two callers is which tests they feed in: every test of the file for `classify_red`, and the matched tests for `classify_named`. The callers build the results with one checked `jq` call (`JQ_FAILURE_RESULT`) that turns a `null` `failureMessages` into an empty message, and `classify_failures` refuses a test with an empty message by name, and a call with no results at all, instead of defaulting to `assertion` (review finding 1 and the Copilot review).
...
- `claude/enforce/tests/tdd-red-green.test.sh`: the same case under the real Vitest, where one test fails `toBe` and one throws `new Error("boom")`, must be refused as `src/__tests__/score.test.ts::throws boom fails for a reason this script does not classify: Error: boom`. A new Jest-stub mode reports two failing results under the named test, an assertion followed by one with `failureMessages: null`, and `tdd.sh red` must refuse the second by name with `failed with no failure message to classify` rather than lock the file on the first.
...
Copilot found that the first fix for review finding 1 was incomplete. `jq` streams its output, so when a result with `failureMessages: null` came after a readable one, the readable result was already on the stream when `jq` failed, the count floor saw one result, and the file was locked on that result alone. Fixed test-first: the Jest-stub case now puts an assertion result ahead of the null one (it was accepted before the fix), the producer is null-safe and its exit status is checked with `die`, and an empty message is refused by name.
