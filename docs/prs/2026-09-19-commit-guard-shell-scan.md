...
- Two new fixtures: `claude/enforce/tests/commit-message-guard-shell-scan.test.sh` (the data-versus-command cases from the incident, and real commits behind wrappers and global options) and `claude/enforce/tests/commit-message-guard-review-cases.test.sh` (the cases the pre-merge review found, below). `claude/enforce/hook-hashes.txt` is regenerated. The Codex and Cursor ports regenerate with no changes.
