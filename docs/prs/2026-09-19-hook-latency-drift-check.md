...
- **A git-quoted tracked name counts as drift.** Copilot's second round noted that such a name matched no payload prefix and was silently skipped. The per-file loop counted it as missing, so the batched check now does the same rather than switching to `ls-files -z`, because `sync.sh` itself lists files without `-z` and cannot install such a name either; a quiet skip would hide that, and a drift report does not. The temporary directory is created from an explicit `${TMPDIR:-/tmp}/harness-sync.XXXXXX` template, and failing to create it counts every pair as drift.
...
- After the review, the process-counting case run against `origin/main`'s hook printed `started 1043 cmp or git process(es) (budget 20)` and `FAIL: the no-drift check does not start a process per tracked file`; case 3h run against the first batched version printed `FAIL: a newline home in sync is not drift`. Case 3i failed against the second head with `FAIL: a git-quoted tracked name is not silently skipped`. With the final hook the file prints `started 5 cmp or git process(es) (budget 20)` and `harness-sync.test.sh PASS`, and the no-drift run on the real checkout takes 130 to 180 ms.
