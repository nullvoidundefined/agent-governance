...
- **The lock is owned by a PID, not expired by age.** Copilot's review of this PR found that the ten-minute expiry cherry-picked from #55 could reclaim the lock under a slow but live `npm ci`, letting two installs rebuild the same `node_modules`. The holder now writes its PID into the lock, and another caller reclaims it only when `kill -0` shows that process is gone. A lock with no PID file is treated as held. Writing that fixture also exposed a hang in the old expiry: it removed the directory with `rmdir`, which cannot remove a directory holding a file, so it would have looped forever.
...
- `sync-tests/sync.test.sh` gains a case where an old lock held by a live PID must be waited on, and a lock left by a dead PID must be reclaimed. Against the expiry code it hung, which is the red result.
