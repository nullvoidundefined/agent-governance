...
0. **Linear is at its free issue limit.** The seventeenth ticket of the IAN-268 batch was refused outright. Facet 4's content (the retirement rule, the hash manifest, the distribution redesign) is a comment on IAN-268 and needs promoting once there is room. **Every new ticket fails until this is resolved**, which also means R-605 cannot be satisfied for new work.
...
## 7. The IAN-218 backlog-capture session (folded from #101)

Folded in when `#101` was cleared, because `main` carried this session's PR numbers but none of its substance: IAN-204, IAN-205 and IAN-219 appeared nowhere, and three findings would have been lost with the branch.

- **Shipped:** `docs/tickets/2026-09-20-track-and-release-backlog.md` (sixteen work items, four workstreams) and a `linear` block in `claude/TICKET-TRACKER.template.json`. IAN-202 to IAN-217 opened; IAN-218 closed at 76 actual against 90, ratio 0.84, rework 1.
- **The rework, worth keeping:** the R-605 gate was disabled the whole session because `~/.claude/TICKET-TRACKER.json` was absent, then activated the instant that file was written and refused a commit after four had already landed. A gate that depends on a gitignored per-machine file is off by default on every fresh checkout, which is the same shape as the guards that were failing open on macOS.
- **Carried forward, still true:** that tracker config was written in an ephemeral container and never reached the laptop. Copy the `linear` block from the template and fill in team and project locally, or R-605 stays disabled there.
- **IAN-219:** the MCP permission layer does not port. Cursor runs the R-105 guard but never evaluates an `mcp__*` allow entry, and whether Codex reaches the guard at all is unverified. That answer decides whether this is an ergonomics gap or a security one.
- **IAN-204:** `sync.sh:161` is `rsync -a --checksum` with no `--backup`, so a live file that is still tracked upstream and has been edited locally is overwritten without notice at every SessionStart.
- **IAN-205:** `CLAUDE-PYTHON.md` serves 1000 lines of FastAPI conventions to every Django repository and names Django zero times. Needs an add-or-narrow decision before it can be scheduled.
- The `linear` block never existed in the template despite Linear being the live tracker since at least IAN-121. Its convention (fields in a fenced `ticket-fields` description block, four canonical states carried as labels because Linear has no custom fields) was reconstructible only by reading existing issues.
