...
- GitHub: ruleset `copilot-auto-review` on the default branch requires a PR and squash merge. Copilot code review is now active on the repo (the owner enabled it): it reviewed PR #2 automatically and on request. Standing rule from the owner, recorded in project memory and slice 02 PR 3: after opening a PR, poll for Copilot's review, verify each comment, fix the valid ones, reply to the rest, resolve threads with the SHA, re-request at most once, stop after two rounds or a round with no valid finding.
