...
| 3 | Skill pointers, ticket-lifecycle Linear row, repo-setup skill, Copilot review loop, README, sync, hashes, handoff | 23% | | | Adds the Linear provider row, the project-per-repository rule, the existing `repo-setup` skill (merged to `main` in #3 during this session) extended with a `require-pr-squash` ruleset on the default branch, a `copilot-review-all-branches` ruleset carrying the `copilot_code_review` rule with `review_on_push` true, and the Linear project, and the Copilot review-loop rule in build-by-slice (poll for the review after opening a PR, verify each comment, fix the valid ones, reply to the rest, resolve threads with the SHA, re-request at most once; stop after two rounds or a round with no valid finding), all raised 2026-09-17 during slice 01 |
